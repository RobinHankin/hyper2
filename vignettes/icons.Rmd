...
use $p_1>1/6$ as an example but the method applies to any observation
with a natural one-sided alternative.
...
* We may either use Edwards's criterion of two units of support per
  ...
  $H_0\longrightarrow 2\Lambda\sim\chi^2_1$.  Here, because we are
...
Note that the above protocol works only for inequality hypotheses; it
is not applicable to hypotheses without natural one-sidedness.  For
example, consider $H_0\colon p_1=p_2=p_3$.  The vector ${\mathbf p}\in
\left\{\left.\left(p_1,\ldots,p_6\right)\right| \sum p_i=1\right\}$ is
a point in a 5-dimensional manifold, and $H_0$ asserts that ${\mathbf
p}\in \left\{\left.\left(p_1,p_1,p_1,p_4,p_5,p_6\right)\right|
3p_1+p_4+p_5+p_6=1\right\}$, that is, a 3-dimensional manifold.  The
most common example of this would be the hypothesis of player
...
("E" for equal).  Testing $H_E$ is implemented in the package as
