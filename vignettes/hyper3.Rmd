...
@hankin2017_rmd; to cite `hyper3` functionality, use @hankin2024.
...
The `hyper2` package [@hankin2009,hankin2017] furnishes computational
support for generalized Plackett-Luce [@plackett1975] likelihood
functions.  The preferred interpretation is a race (as in track and
field athletics): Given six competitors $1-6$, we ascribe to them
nonnegative strengths $p_1,\ldots, p_6$; the probability that $i$
...
However, the majority of software is written in the R
computer language [@rcore2023], which includes extensive
functionality for working with such likelihood functions:
@turner2020 discuss several implementations from a computational
perspective.  The `BradleyTerry` package [@firth2005]
...
package [@turner2012] implements a flexible user interface and
...
[@turner2020] generalizes this to likelihood functions of the
form of Equation \ref{PL_like} and applies the Poisson transformation
of [@baker1994] to express the problem as a log-linear model.  The
...
statistics on the $p_i$ [@hankin2017].  It becomes possible to
...
draws [@hankin2009], noncompetitive tactics such as
...
Existing \code{hyper2} formalism as described by @hankin2017
...
\code{hyper2} presented by @hankin2017 and reviewed
...
criterion [@edwards1972}, and we conclude that our observation
...
functions, modified from @hankin2017 to work with \code{hyper3}
...
lower and upper limits of the credible interval; Figure
\ref{ABA_likelihood}.
...
Above, object `javelin` is a `hyper3` likelihood function,
...
with `equalp.test()` reveals that there is no strong evidence in
...
training or selection perspective we might follow [@hankin2017]
and observe that log-contrasts [@ohagan2004] appear to have
...
# Formula 1 motor racing: The Constructors' Championship}\label{f1
...
Plackett-Luce probabilities [@hankin2017]; or the introduction of
