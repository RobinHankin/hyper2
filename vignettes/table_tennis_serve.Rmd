...
profile_likelihood <- function(S,give=FALSE){
  ...
  objective <- function(pab){      # pab=c(a,b); S in scope (c is the fillup)
    loglik(H,c(S,pab[1],pab[2]))   # no minus because we use fnscale= -1
  }
  
  maxlike_constrained <-           # 2 DoF, a,b [S is given, c is fillup]
    constrOptim(
        f = objective,
        theta=c(rep((1-S)/3,2)),   # start at indep([c(S,x,x,x)])
        grad=NULL,
        ui=rbind(diag(2),-1),      # pa,pb>=0, pa+pb <1-S (pc is the fillup)
        ci=c(0,0,S-1),             # ditto
        control=list(fnscale = -1) # finds max, not min
    )
