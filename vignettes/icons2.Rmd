...
presented in the hyperdirichlet R package, but using the hyper2
package instead.
...
Each row of ```M``` corresponds to a particular cue given to
respondents.  The first row, for example, means that a total of
$5+3+4+3=15$ people were shown icons NB, L, TCH, WAIS [column names of
the the non-NA entries]; 5 people chose NB as most concerning, 3 chose
L, and so on.  The dataset is more fully described in the
hyperdirichlet package.

To recreate the ```icons``` dataset in the ```hyper2``` package, we
proceed as follows.  First set up a hyper2 object with the correct
names:
...
The  iterate through each row to work out the denominators:
...
constraint that $p_1\leqslant 1/6$.  This is most easily done with the
```constrOptim()``` function for constrained optimization:

```{r}
small <- 1e-6  #  ensure start at an interior point
maxlike_constrained <-
    constrOptim(
        f = function(p){print(p);loglik(icons,p)},  
        theta=indep(equalp(icons))-c(small,0,0,0,0),
        grad=NULL,
        ui=rbind(nonneg=diag(5), -1, c(-1,0,0,0,0)), # p1-p5; fillup; p1
        ci=c(rep(0,5),-1,-1/6), # nonneg; nonneg; <1/6
...
constrained optimizations: $p_1\leq p_2, p_1\leq p_3, p_1\leq p_4,
p_1\leq p_5$.  The fillup constaint would be $p_1\leq
p_6\longrightarrow 2p_1+p_2+\ldots p_5\leq 1$.  We then choose the
largest likelihood from the five.

```{r}
o <- function(Ul,Cl,give=FALSE,start){
    small <- 1e-6  #  ensure start at an interior point
    if(missing(start)){start <- indep(c(small, rep(0.1,5)))}
    out <- constrOptim(
        f = function(p){loglik(icons,p)},  
        theta=start,
        grad=NULL,
        ui=rbind(nonneg=diag(5), -1, Ul),
        ci=c(rep(0,5),-1,Cl), 
        control=list(fnscale = -1)  # find max, not min
    )
...
In the above, the idea is that we implement the additional constraint
($p_1\leq p2$ etc) by passing ```Ul``` and ```cl``` which are the last
row of ```Ui``` and vector ```Ci``` respectively.
...
The next hypothesis was $p_5+p_6\geqslant\frac{1}{3}$.  This translates
into optimizing subject to $-p_1-p_2-p_3-p_4\geqslant\frac{-2}{3}$.  Dead
easy:
...
\[
\bigcup_{i=1}^4(p_5\geqslant p_i)\cup(p_6\geqslant p_i)
\]
