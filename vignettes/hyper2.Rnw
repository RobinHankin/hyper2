...
Here I present the \pkg{hyper2} package for generalized
...
[the second argument of function \code{lhyper2()} is a vector of
...
defined likelihoods for observations that have a peculiar probability
structure, and I give two examples below.
...
\code{masterchef\_series6}.  We may assess a number of related
hypotheses using the package.  The first step is to calculate the
likelihood for the hypothesis that all players are of equal strength:
