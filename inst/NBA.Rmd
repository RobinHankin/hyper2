...
for(i in seq_len(nrow(NBA_table))){
  ...
  if(possession[i] == 'W'){
    onpitch_scored <- c(onpitch_scored,"W_possession")
  } else if (possession[i] == 'C'){
    onpitch_scored <- c(onpitch_scored,"C_possession")
  } else {
    stop("possession must be either W or C")
  }
  NBA[onpitch_scored] %<>% inc()
  NBA[c(onpitch_scored,onpitch_noscore)] %<>% dec()
}

NBA_maxp <- maxp(NBA)
NBA_maxp %<>% ordertrans(c(colnames(NBA_table)[-(1:4)],"C_possession","W_possession"))
dotchart(NBA_maxp,col=c(rep("black",9),rep("red",10),rep("blue",2)),pch=16)
```

# Statistical analysis

```{r,statsanal,cache=TRUE}
samep.test(NBA,c("C_possession","W_possession"))
```

We may thus assume that Cleveland having possession is the same
strength as the Warriors having possession.
