...
<font size="1"> (takes about ten minutes run without cache)
</font>

Here I use the concept of _dominated set_ to simplify a Plackett-Luce
likelihood function.
...
The dataset comprises seven observations covering years 2015-2022.
Each year, ten or eleven cities are ranked in order of liveability,
according to a number of criteria such as prevalence of crime,
...
We see that NZ and Australia are a dominated set [that is, they come
last in every race], so we should remove them:

```{r}
(o_countries <- lapply(o_countries, function(x){x[!(x %in% c("NZ","Australia","Japan"))]}))
```

The next dominated set to remove would be Canada, Germany,
Netherlands; and then Switzerland.  This leaves us with Austria.  So
