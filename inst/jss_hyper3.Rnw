...
\Abstract{
  ...
  The \code{hyper2} package furnishes extensive idiom to work with
  extensions of the Bradley-Terry probability model such as
  Plackett-Luce likelihood functions, team strengths, and reified
  entities (monsters).  Here, I present a generalization of
  \code{hyper2} in which multiple entities are constrained to have
  identical Bradley-Terry strengths.  A new \proglang{S3} class
  \code{hyper3}, along with associated methods, is motivated and
  introduced.  Four datasets are analysed, each analysis furnishing
  new insight, and each highlighting different capabilities of the
  package.
}
...
intractable and numerical methods must be used.  Given an order
statistic of the type considered above, the Mann-Whitney-Wilcoxon
...
would estimate the probability that competitors, say Vadlejch would
throw further than Vesely at $\displaystyle p_{\mbox{\tiny
Vad}}/\left(p_{\mbox{\tiny Vad}} + p_{\mbox{\tiny Ves}}\right)\simeq
0.74$.  Indeed, from a training or selection perspective we might
follow \cite{hankin2017} and observe that log-contrasts
~\citep{ohagan2004} $\log\left(p_{\mbox{\tiny Vad}}/ p_{\mbox{\tiny
Ves}}\right)$ appear to have approximately Gaussian likelihood
functions for observations of the type considered here.  Profile
log-likelihood curves for log-contrasts are easily produced by the
package, Figure~\ref{profliklogcont}.
...
championship, which is between drivers).  Here, we consider the
constructor to be the object of inference.  Each constructor typically
fields two cars, each of which separately accumulates ranking-based
points at each venue.  Here we use generalized Plackett-Luce to assess
the contructors' performance.  The following table, included in the
\code{hyper2} package as a dataset, shows rankings for the first 9
venues of the 2021 season:
...
between 2020 and 2021 was tested and rejected.
...
the characterisation of the power function of both tests.  Further,
the package could easily be extended to allow non-integer
multiplicities, which might prove useful in the context of reified
Bradley Terry techniques~\citep{hankin2020}.
