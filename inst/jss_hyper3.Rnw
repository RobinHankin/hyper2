...
\Abstract{
  ...
  introduced.  Three datasets are analysed, each analysis furnishing
...
computer language~\citep{rcore2022}, which includes extensive
...
using generic function \code{maxp()}, dispatching to a specialist
\code{hyper3} method:
...
encountered result formats; an example is given below.
...
legend(x=0.85, y=0.7, col = c("black", "red", "blue"), lty=1, pch=NA,
       legend=c("AAB", "ABA", "BAA"))
...
\begin{figure}[htbp]
  \begin{center}
<<plotwilcoxlike,echo=FALSE,fig=TRUE>>=
...
\begin{figure}[htbp]
\begin{center}
<<plottheloglikcont,echo=FALSE,fig=TRUE>>=
