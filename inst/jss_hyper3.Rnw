...
are integers which may be positive or negative.  The \pkg{hyper2}
package has the ability to evaluate such likelihood functions at any
point in $S_n$, thereby admitting a wide range of non-standard nulls
such as order statistics on the $p_i$~\cite{hankin2020}.  It becomes
possible to analyse a wider range of likelihood functions than
standard Plackett-Luce~\citep{turner2020}.  For example, results
involving incomplete order statistics or teams are tractable.
Further, the introduction of reified entities (monsters) allows one to
consider draws~\citep{hankin2009}, noncompetitive tactics such as
...
use generalized a Plackett-Luce model to assess the constructors'
