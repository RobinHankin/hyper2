...
```{r}
(soling_diff <- soling_after + (-1)*soling)
```

So `H2` is the _difference_ between `H`--- which is `soling` plus the
observations from the roundrobins and quarter finals---and `soling`
withouth the extra information.  In other words, `H2` is the simply
the information in the roundrobins and quarter finals.
