...
```{r}
 ...
 L <- f(a,r=c(3,4,3,3,1,3,3,3,1,1,1,5),10,log=T)  # 10+1==11 competitors in total
plot(a,L-max(L))
...
plot(NA,xlim=c(-5,1),ylim=c(-5,0),type='n',xlab="log-odds")
abline(v=-log(22))
for(i in seq_len(23)){points(log(a/(1-a)),fun(i),type='l')}
```
...
abline(v=-log(22))
for(i in seq_len(20)){points(log(a/(1-a)),fun(i),type='l')}
...
plot(NA,xlim=c(-5,1),ylim=c(-5,1),type='n',xlab="log-odds")
abline(v=-log(99))
for(i in seq_len(40)){points(log(a/(1-a)),fun(i),type='l')}

```{r}
x <- as.matrix(read.table("ol.txt",header=TRUE))[,1:4]
x <- x[apply(x,1,max) <= 100,]
fun <- function(i){
 jj <- as.numeric(unlist(x[i,]))
  L <- f(a,jj,100,log=TRUE)
