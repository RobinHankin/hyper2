...
```{r optimsinglelike}
(optparkrun <- optimize(function(a){L(a,results)},c(0.2,0.8),maximum=TRUE))
f <- function(a){L(a,results)-2}
```

```{r plotparkrunlike, fig.cap="Support curve for author's generalized Bradley-Terry strength.  Evaluate of $\\hat{a}\\simeq 0.448$ has a support of zero"}
plot(a,La,xlab="generalized Bradley-Terry strength",ylab="support")
abline(h=c(0,-2))
abline(v=0.5)
```
...
$\mathcal{S}(\hat{a})=0$.  We see a credible interval [that is, values
of $a$ for which $\mathcal{S}(a)\geqslant -2$] of about 
