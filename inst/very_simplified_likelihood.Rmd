...
```{r}
support_vec <- function(a,ranks,runners){
    out <- 0
    for(i in seq_along(ranks)){
        out <- out + f_single(a, ranks[i],runners[i],log=TRUE)
    }
    return(out)
}

support_ij <- function(v){
    auck <- v[1]
    stir <- v[2]
    return(
        support_vec(auck, rank_auck, runners_auck) +
        support_vec(stir, rank_stir, runners_stir)
    )
}

x <- seq(from=0.1,to=0.8,by=0.01)
jj <- as.matrix(expand.grid(x,x))
L <- apply(jj,1,support_ij)
L <- L - max(L,na.rm=TRUE)
L <- matrix(L,length(x),length(x))
```
