...
\[
\frac{(B-1)\Gamma(B+n-r-1)}{\Gamma(B+n)}
\]
...
f_single <- function(a,r,n,log=FALSE){  # not vectorised
  ...
  if(log){
     if(r==n){  # last place
       ...
       out <- log(B-1) - sum(log(B + (n-r-1):(n-1)))
     }
   } else {  # not-log 
     if(r==n){
...
f_single2 <- function(a,r,n,log=FALSE){  # not vectorised
  B <- 1/(1-a)
  if(log){
     if(r==n){  # last place
       out <- lgamma(B)-lgamma(B+n)
     } else {   # not last
       out <- log(B-1) - lfactorial(B+n-1) + lfactorial(B+n-r-2)
     }
   } else {  # not-log 
     if(r==n){
      out <- gamma(B)/gamma(B+n)
...
a <- seq(from=0.005,to=0.6,by=0.005)
...
a <- seq(from=0.1,to=0.45,by=0.01)
alike <- sapply(a,f)
plot(a,alike-max(alike,na.rm=TRUE),type='b')
abline(h=c(0,-2))
