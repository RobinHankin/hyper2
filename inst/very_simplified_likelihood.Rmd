...
L <- function(a,results){
   ...
   return(out-max(out,na.rm=TRUE))
}
```

```{r calculatelike,cache=TRUE}
...
```{r}
print(supp2(c(0.1,0.3),results$place,results$runners))
supp2(c(0.1,0.3),results$place,results$runners)
x <- seq(from=0.1,to=0.5,by=0.005)
...
par(pty='s')
contour(x,x,L,asp=1,xlim=range(x),ylim=range(x))
abline(0,1)
```
