...
After simplifying and taking an asymptotic expansion we find

\[
\mathcal{S}=\log\mathcal{L}_{r,n}(a)=\log(B-1)-B\frac{r+1}{n} + \frac{B^2-B(5+r^2)/2}{n^2}
+ K + \mathcal{O}\left(n^{-3}\right)
\]

(where $K$ is an arbitrary constant).  If we then neglect terms of
order $n^{-2}$ and above, viz $\log(B-1)-B\frac{r+1}{n}$, observing
that $\partial^2\mathcal{S}/\partial B^2=-(B-1)^{-2}<0$, the evaluate
would be $\hat{a}=\frac{n}{n+r+1}$ [although this seems to be a poor
approximation to the exact evaluate]
...
MLEa <- function(r,n,give=FALSE){
	out <- uniroot(f=function(a){fdash(a,r,n)},interval=c(0.01,0.99))
...
```{r usestirling}
rbind(
c(MLE(10 , 100),100 / 111),
c(MLE(100,1000),1000/1101),
c(MLE(500,5000),5000/5501)
)

rbind(
c(MLEa(10 , 100),100 / 111),
c(MLEa(100,1000),1000/1101),
c(MLEa(500,5000),5000/5501)
)
```
...
a <- seq(from=0.005,to=0.75,by=0.01)
