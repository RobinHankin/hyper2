...
winning about 37.5\% of games (black wins 27.6\%); and secondly, many
chess games, about 39.4\%, are drawn (figures from Chessbase).
...
chess players.  Ignoring the effect of playing white, we wish to find
numbers (``strengths") $p_i$ for $1\leqslant i\leqslant n$ with
$p_i\geqslant 0, \sum p_i=1$ such that the probability of $i$ beating
$j\neq i$ is $\frac{p_i}{p_i+p_j}$.  A suitable likelihood function,
on the assumption of independence of trials, would be
...
``draw monster'') that wins if the game is drawn.  The system is
...
of drawing.  If we consider a chess game to be a three-way trial, an
appropriate likelihood function would be for non-negative
$p_1,\ldots,p_n,D$ ($D$ being the draw monster's strength), with
$D+\sum p_i=1$ and
...
advantage.  We thus consider another entity whose strength $W$ is
added to the white player.  The strength of the white player is thus
the sum of the player's strength and $W$.  Recalling that a game might
be a draw, we obtain the probabilities as given in
...
of Support~\citep{edwards1972}.  To test any hypothesis $H$ on
...
reified entities representing proclivity to draw $D$, and white or
first-mover advantage ($W$).
...
R~\citep{rcore2019} \pkg{hyper2} package~\citep{hankin2017} as a
...
owing to the nonzero term in the numerator of
equation~\ref{kka_likelihood}.  From the figure, we see that the
maximum likelihood estimate for draw strength $\hat{D}\simeq 0.53$:
compare the na\"ive estimate of $\frac{224}{365}\simeq 0.61$ given by
observing 224 draws among 265 games.  The na\"{i}ve estimate is lower
because the maximization process automatically adjusts for the
different players' strengths.  Secondly, we may make inferences about
$W$, the extra strength conferred by playing white: it is conceivable
that $W=0$ as this term does not appear on its own in the numerator of
any term.  We may test $H_0\colon W=0$ using the Method of Support by
maximizing the likelihood subject to the constraint $W=0$ and
comparing with the likelihood when the optimization is free.
...
\noindent for the numerator (where $C$ is the strength of a collusive
...
$\log\left(D_s/D\right)$ which, if Dirichlet, would be accurately
