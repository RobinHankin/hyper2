...
moul: White's relative advantage over Black will be dependent upon the
comparative mastery of the two players.

The most obvious issue regarding the threshold against which we compare
this relative advantage of White is the inherent advantage of White's
position in moving first. We correspondingly do not expect these base
thresholds to be symmetric and impose no such restriction.
...
non-competitive situations such mating preference~\cite{hankin_aylmer}
and consumer behaviour.  The Bradley-Terry model~\citep{bradley1952}
...
multi-way choices such as icon selection~\cite{hankinhyperdirichlet}.
...
as they are generally poorer in spectacular or striking elements such
as sharp tactical combinations or sacrifice.  Excessive drawing
dissuades sponsors, leaves spectators feeling cheated, and leaves
commentators with material to discuss with the public.
\cite{moul2009} concurs, conceptualising a player as being able to
adopt a strategy on a spectrum ranging from conservative to
aggressive, thus being able to control the probability of a draw.  He
suggests that risk-averse players may pursue less vigorous strategies,
trading expected payout for reduced variance.   

such as chess, the Bradley-Terry model can
be augmented to chess-specific considerations in a number of ways.
Here I present a new generalization of Bradley-Terry in which a chess
match is regarded as a three-way competition between the two players
and an entity that wins if the match is drawn.  Bradley-Terry can be
further modified to account for the White player advantage in a number
of ways.  This methodology affords insight into players' strengths,
response to playing Black or White, and risk-aversion as manifested by
probability of drawing.  The likelihood functions arising are easily
optimised numerically.  I analyse a number of datasets of chess
results, including the infamous World Chess Championships 1963 in
...
competition between the two ``real'' players and the draw monster, but
this time we interpret the white player as the combined strength of
the player himself and another entity, the White monster, whose
strength $p_W$ is added to the white player.
