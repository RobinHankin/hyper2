...
effectively uses, in current notation, $D=\nu\sqrt{p_ip_j}$.  However,
\cite{joe1990} uses likelihood methods to show that this model does
not fit data well.  Here we note that such a model automatically give
stronger players a higher draw potential, and also note that it is
difficult to account for first-player advantage in this framework.
...
consider one famous incident, following the World Chess Championships
1963, in which Fischer famously and publicly claimed that the Soviet
players colluded.  The accusation was that Soviet players agreed ahead
of time to draw when playing one another, in order to conserve energy
for subsequent matches.

\citet{moul2009} consider this accusation from a statistical
perspective and present a detailed analysis.  Unlike the present work,
which uses a dataset restricted to game result (win/loss/draw),
\citet{moul2009} included number of moves in each game\footnote{The
  reasoning was that hard-fought games that happened to end in a draw
  are more likely to last longer than collusive or pre-arranged
  draws.} in their analysis, and used a dataset of 7355 games played
in the period 1940-1978 to calibrate the ability of the players.  In
contrast, the analysis presented uses a dataset restricted to game
outcome (win-loss-draw) in the competition itself.
...
1962 Candidates (match results shown in Table~\ref{candidates}).
Fischer alleged that three of the Soviet players (Keres, Petrosian,
and Geller) had agreed to draw amongst themselves.  Observe that all
twelve of the games between the three accused players ended in a draw,
a significant result given 51 winning games and 54 draws (Fisher exact
test, one-sided, $p=0.0079$).  

To investigate the possibility of Soviet collusion we introduce a new
draw entity: a soviet draw monster $C$, who wins when two colluding
players draw, and is inactive whenever a game has at least one
non-colluding player.  Then $H_N\colon C=D$ corresponds to no
collusion and the likelihood function would be
...
$\left(P+\mathit{Ke}+W+C\right)^{4}$ [indicating that Petrosian played
  Keres 4 times] on the denominator.  The likelihood ratio for the
...
The fith interzonal took place in Stockholm in 1962, in the form of a
23-player round-robin tournament.  For the results shown in
...
investigate the possibility of increased Soviet drawing proclivity, we
introduce a Soviet draw monster $D_s$, who wins when two Soviet
players draw, and is inactive if a game has at least one non-Soviet
player.  Then $H_N\colon D_s=D$ corresponds to no collusion and the
likelihood function for table~\ref{interzonal} would be
