...
\section{Chess collusion: Cura\c{c}ao 1962}
...
now consider one famous incident, the World Chess Championships 1963,
in which Fischer famously and publicly claimed that the Soviet players
colluded.  The accusation was that Soviet players agreed ahead of time
to draw when playing one another, in order to conserve energy for
subsequent matches~\citep{moul2009}.


%the question of Soviet collusion during the period when the USSR
%dominated the struggle for the world championship has been one of the
%long-standing debates in the chess world.  The best-known (but not
%unique) criticism was aired by Bobby Fischer some weeks after the 1962
%Candidates Tournament in Curacao
...
% Keres     &USSR    &             &NA      &DDDD   &LDWD   &DDWD   &WWWL   &WDW    &DWWD\\
% Geller    &USSR    &             &NA      &NA     &WWDL   &DDWD   &DDDW   &DWW    &DWWD\\
% Fischer   &USA     &             &NA      &NA     &NA     &LWLD   &LWDW   &DWD    &WDWD\\
% Korchnoi  &USSR    &             &NA      &NA     &NA     &NA     &DDDL   &WLD    &WWWW\\
% Benko     &USA     &             &NA      &NA     &NA     &NA     &NA     &WLD    &LWWD\\
% Tal       &USSR    &             &NA      &NA     &NA     &NA     &NA     &NA     &WLD \\
% Filip     &TCH     &             &NA      &NA     &NA     &NA     &NA     &NA     &NA \\
% \end{tabular}
% \caption{Cura\c{c}ao 1962 candidates, world championship 1963}
% \end{table}


\section{Chess collusion: interzonal Stockholm 1962}
