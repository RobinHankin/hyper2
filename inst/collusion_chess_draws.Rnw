...
which the Soviets were accused of collusion.  I conclude that the
scoreline of the Candidates Tournament (Cura\c{c}ao 1962) exhibits no
evidence of collusion, in agreement with previous work.  However, I
find strong evidence of increased Soviet proclivity to draw in the
Interzonal Tournament, Stockholm 1962.
}
...
replayed and in this context we note that this procedure reduces a
chess game to a Bernoulli trial, with probability one.  This system
became unworkable due to the large number of draws, resulting in the
modern practice of awarding each player of a drawn game half a point,
rendering chess a zero-sum game.  Drawn games are generally considered
to be undesirable as they are usually poorer in spectacular or
striking elements such as sharp tactical combinations or sacrifice.
Excessive drawing dissuades sponsors, leaves spectators feeling
cheated, and gives commentators less material to discuss.  Players
themselves do not seem to value draws in the wider context of the
game: \citeauthor{fischer1969}, for example, includes only 8 draws in
his compilation of 60 memorable games, surely not representative of
tournament play in which about 39.4\% of games are drawn (binomial
test, $p<10^{-5}$).
...
previous likelihood functions of draws in that they include entities
representing proclivity to draw $D$, and first-mover advantage ($W$).
...
on the numerator (where $C$ is the strength of a collusive draw), and
a total of 36 terms such as $\left(P + \mathit{Fis}+W+D\right)^{4}$
and $\left(P + \mathit{Ke}+W+C\right)^{4}$ on the denominator.
...
The maximum likelihood estimate is $D\simeq 0.043, D_s=0.354$; a constrained maximization gives a likelihood ratio of about 5096, or a $p$-value of about $3.6\times 10^{-5}$.  Indeed we could go further and plot a profile likelihood curve for the log-odds ratio $\ln\left(D_s/D\right)$ which, if Dirichlet, would be accurately parabolic over a wide range (OHAGAN AND FORSTER, p343).  This is illustrated in Figure~\ref{DOESNOTEXIST}, that shows a credible interval of about HAVENOTDONETHISET.
