...
to chess-specific considerations in a number of ways.  Here I present
a new generalization of Bradley-Terry in which a chess game is
regarded as a three-way competition between the two players and an
entity that wins if the game is drawn.  Bradley-Terry can be further
modified to account for the white player advantage in a number of
ways.  Such reification affords insight into players' strengths,
response to playing black or white, and risk-aversion as manifested by
probability of drawing.  The likelihood functions arising are easily
optimised numerically.  I analyse a number of datasets of chess
results, including the infamous World Chess Championships 1963 in
which the Soviets were accused of collusion.  I conclude, on the basis
of a dataset that includes only the scorelines at the event itself,
that the scoreline of the Candidates Tournament (Cura\c{c}ao 1962)
exhibits evidence of collusion ($p<10^{-5}$), in agreement with
previous work that uses additional information such as historical
results including number of moves.}
...
Bernoulli trial.  The canonical application would be to two-way
competitive situations such as tennis in which one or other player is
idenfied as the winner, but the system has been applied to many
non-competitive situations such mating preference~\citep{west2008} and
consumer behaviour.  The Bradley-Terry model~\citep{bradley1952}
considers $n$ players and assigns non-negative numbers
$p_1,\ldots,p_n$ with unit sum to each player.  Then player $i$ beats
player $j$ with probability $\frac{p_i}{p_i+p_j}$.  This simple system
has been modified and extended many times, for example to account for
multi-way choices such as mating preferences~\citep{hankin2010}.
...
and secondly, many chess games end in a draw: white wins about
$37.5\%$ of all games, black wins $27.6\%$, and about $39.4\%$ are
drawn.  We consider the first-mover advantage first, then consider the
effect of drawn games.

\paragraph{First-mover advantage} Traditionally White moves first in
any game, and this feature introduces an asymmetry into play.
...
\paragraph{Draws} Chess games may be won by either player, or may be
drawn; the implication of a draw is that neither player is better than
the other.  Before 1867, tournament games that were drawn were
replayed until one or other player won, thus reducing chess game to a
Bernoulli trial, almost surely.  This system became unworkable due to
the large number of draws, resulting in the modern practice of
awarding each player of a drawn game half a point, rendering chess a
...
draws in the wider context of the game: \citeauthor{fischer1969}, for
...
chess context, this means the leading player will aim for a draw.

Such arguments suggest considering a drawn chess game as a
qualitatively different outcome from a statement of equality of skill:
the play itself tends to be different, the drawing players have a
different mindset, and drawn games have less value in the wider
context of the game.

\section{Generalized Bradley-Terry}

Consider first the standard Bradley-Terry model for a finite number of
chess players.  First we ignore the effect of playing white.  We wish
to find numbers (``strengths") $p_i$ for $1\leqslant i\leqslant n$
with $p_i\geqslant 0, \sum p_i=1$ such that the probability of $i$
beating $j\neq i$ is $\frac{p_i}{p_i+p_j}$.  A suitable likelihood
function, on the assumption of independence of trials, would be
...
generalization of Bradley-Terry in which a chess game is regarded as
a three-way competition between the two players and an entity that
wins if the game is drawn.  The system is further modified to account
for first-move advantage by positing an entity whose strength is added
to that of the white player.  This methodology affords insight into
players' strengths, response to playing black or white, and
risk-aversion as manifested by probability of drawing.

If we consider a chess game to be a three-way trial, we define a
likelihood function for non-negative $p_1,\ldots,p_n,D$ with
$D+\sum p_i=1$ and this would be

\begin{equation}
\prod_{i<j}
\left(\frac{p_i}{p_i+p_j+D}\right)^{W_{ij}}
\left(\frac{p_j}{p_i+p_j+D}\right)^{L_{ij}}
\left(\frac{D  }{p_i+p_j+D}\right)^{D_{ij}}
\end{equation}
...
advantage.  We thus consider another entity whose strength is added to
the white player.  The strength of the white player is thus the sum of
the player's strength and another entity, the White Monster.
Recalling that a game might be a draw, we obtain the probabilities as
given in Table~\ref{probabilities}.  The corresponding likelihood
function is given below in equation \ref{likelihood_WD}:
...
$\Prob{(\mbox{$i$ wins})}$  & $\frac{p_i+W\oo}{p_i+p_j+W+D}$ & $\frac{p_i}{p_i+p_j+W+D}$  \\
$\Prob{(\mbox{draw})}$      & $\frac{D\oo}{p_i+p_j+W+D}$     & $\frac{D}{p_i+p_j+W+D}$    \\
$\Prob{(\mbox{$i$ loses})}$ & $\frac{p_j\oo}{p_i+p_j+W+D}$   & $\frac{p_j+W}{p_i+p_j+W+D}$\\
\end{tabular}
\caption{Probabilities of a win, draw, loss for the two players under a generalized Bradley-Terry model\label{probabilities}}
\end{table}

\begin{align}\nonumber
\prod_{i<j}&{}
\left(\frac{p_i+W}{p_i+p_j+D+W}  \right)^{W_{ij}^\mathrm{white}}
\left(\frac{p_j}{p_i+p_j+D+W    }\right)^{L_{ij}^\mathrm{white}}\\
&{} \left(\frac{p_i}{p_i+p_j+D+W}\right)^{W_{ij}^\mathrm{black}}\label{likelihood_WD}
\left(\frac{p_j+W}{p_i+p_j+D+W  }\right)^{L_{ij}^\mathrm{black}}\\
&{} \left(\frac{D}{p_i+p_j+D    }\right)^{D_{ij}}
\nonumber
\end{align}
...
\begin{equation}
\prod_{i<j}
\left(\frac{D}{p_i+p_j+D+W}\right)^{D_{ij}^\mathrm{white}}
\left(\frac{D}{p_i+p_j+D+W}\right)^{D_{ij}^\mathrm{black}}
=
\prod_{i<j}
\left(\frac{p_D}{p_i+p_j+D}\right)^{D_{ij}^\mathrm{white}+D_{ij}^\mathrm{black}}
\end{equation}

which might be preferable on grounds of clarity.  We now apply this
and related likelihood models to real chess datasets.  The general
approach follows~\citet{edwards1972}.  To test any hypothesis $H$ on a
likelihood function $\mathcal{L}$ one first identifies the constraint
on hypothesis space corresponding to $H$.  One then finds the
unconstrained maximum likelihood $\mathcal{L}_\mathrm{free}$ and
compares it with the maximum likelihood $\mathcal{L}_\mathrm{cons}$
when the optimization is constrained by $H$.  The test statistic is
...
representations of chess results.  However, they differ markedly from
previous likelihood functions of draws in that they include reified
entities representing proclivity to draw $D$, and white or first-mover
advantage ($W$).

\subsection{Previous work}

\cite{davidson1970} uses a threshhold approach to interpret a draw as
performance of the players the same to within a threshhold.  Using a
logistic expression for the difference in players' performance, he
effectively uses, in current notation, $D=\nu\sqrt{p_ip_j}$.  Here we
note that such a model automatically give stronger players a higher
draw potential, and also note that it is difficult to account for
...
constraint becomes nonlinear and likelihood maximization becomes
difficult.
...
The analysis presented here uses the framework of the \pkg{hyper2}
package~\citep{hankin2017} as a convenient and unified interface.
Because gradients are available, convergence to the evaluate is
generally rapid and hypothesis-specific constraints on parameter space
are, in general, straightforward to implement.

\section{Anand, Karpov, Kasparov}

Grandmasters Anand, Karpov, Kasparov have long-standing rivalry and
have played many games amongst themselves.  A summary may be found
...
This likelihood function is easily maximized numerically
...
observing 224 draws among 265 games.  The MLE is lower because the
...
that $W=0$ and infer that playing white confers a significant
...
=0.104 and draw =0.53.  Thus we would estimate that Karpov playing
...
is an important and interesting component of the game.  To investigate
...
achieved by considering a game between two players to be a contest
between five entities: the two real players, the white advantage, and
two {\em personalised} draw entities, one for each real player; a
drawn game is counted as a win for either of the draw ``ghosts''.  Under
these assumptions, table~\ref{kka} translates into the following
likelihood function:
...
@ 
\caption{Maximum likelihood estimate for \label{kka_3draws} equation \ref{KKA_personalised_drawmonster} equation} 
  \end{center}
\end{figure}


In figure~\ref{kka_3draws}, we see considerable variability in
proclivity to draw and again this may be tested with the Method of
...
white, is specific to each player and we denote this $w_\mathrm{An}$,
$w_\mathrm{Kr}$ and $w_\mathrm{Ks}$.  The corresponding likelihood
function would be

\begin{equation}\label{KKA_personalised_whitemonster}
%\mathcal{L}\left(
%p_\mathrm{An},p_\mathrm{Kr},p_\mathrm{Ks},
%w_\mathrm{An},w_\mathrm{Kr},w_\mathrm{Ks},D
%\right)=
\frac{
%\parbox{3in}
%       {$
...
p_\mathrm{Ks}^{20}
D^{224}
\left(p_\mathrm{An} + w_\mathrm{An}\right)^{24}%\\ \rule{10mm}{1mm}
\left(p_\mathrm{Kr} + w_\mathrm{Kr}\right)^{25}
\left(p_\mathrm{Ks} + w_\mathrm{Ks}\right)^{45}
%$}
}{\parbox{4in}{$
\left(p_\mathrm{An} + p_\mathrm{Kr} + w_\mathrm{An} +  D\right)^{43}
\left(p_\mathrm{An} + p_\mathrm{Kr} + w_\mathrm{Kr} +  D\right)^{49}\\ \rule{10mm}{0mm}
\left(p_\mathrm{An} + p_\mathrm{Ks} + w_\mathrm{An} +  D\right)^{37}
\left(p_\mathrm{An} + p_\mathrm{Ks} + w_\mathrm{Ks} +  D\right)^{43}\\ \rule{20mm}{0mm}
\left(p_\mathrm{Kr} + p_\mathrm{Ks} + w_\mathrm{Kr} +  D\right)^{99}
\left(p_\mathrm{Kr} + p_\mathrm{Ks} + w_\mathrm{Ks} +  D\right)^{94}
$}}
\end{equation}

In the likelihood function above we see, for example, that Anand won a
total of $18+6=24$ games playing white, $13+2=15$ games playing
Black, and that Anand played white against Karpov a total of
$18+20+5=43$ times.  We fail to reject the null of
$w_\mathrm{An}=w_\mathrm{Kr}=w_\mathrm{Ks}$, having a likelihood ratio
of $4.5$, or an asymptotic $p$-value of $0.076$.  There is no strong
evidence that the players differ in regard to their white advantage.


\section{Collusion in the World Chess Championship 1963}

The method of support allows one to account for proclivity to draw
while accounting for players' strengths and first-move advantage.  We
now consider one famous incident, following the World Chess
Championships 1963, in which Fischer famously and publicly claimed
that the Soviet players colluded.  The accusation was that Soviet
players agreed ahead of time to draw when playing one another, in
order to conserve energy for subsequent matches.

\citet{moul2009} consider this accusation from a statistical
perspective and present a detailed analysis of Cura\c{c}ao.  Unlike
the present analysis, which uses a dataset restricted to game result
(win/loss/draw), \citet{moul2009} included number of moves in each
game\footnote{The reasoning was that hard-fought games that happened
  to end in a draw are more likely to last longer than collusive or
  pre-arranged draws.} in their analysis, and used a dataset of 7355
games played in the period 1940-1978 to calibrate the ability of the
players.  In contrast, the analysis presented uses a dataset
restricted to game outcome (win-loss-draw) in the competition itself.
...
Here I analyse the original subject of the complaint, the Cura\c{c}ao
1962 Candidates, Table~\ref{candidates}.  Fischer alleged that three
of the Soviet players (Keres, Petrosian, and Geller) had agreed to
draw amongst themselves.  Observe that all twelve of the games between
the three accused players ended in a draw, a significant result given
51 winning games and 54 draws (Fisher exact test, one-sided,
$p=0.0079$).  The likelihood function for this dataset would have
...
T^2  % Tal
\mathit{Fil}\,
D^{42}C^{12}\oo\\ \rule{10mm}{0mm}
\left(P+W\right)^3
\left(\mathit{Ke}+W\right)^4
\left(G+W\right)^4\
\left(\mathit{Fis}+W\right)^7\oo\\ \rule{20mm}{0mm}
\left(\mathit{Ko}+W\right)^2
\left(B+W\right)^5
\left(T+W\right)
\left(\mathit{Fil}+W\right)\oo
$}\end{equation}

\noindent on the numerator (where $C$ is the strength of a collusive
draw), and a total of 36 terms such as
$\left(P+\mathit{Fis}+W+D\right)^{4}$ and
$\left(P+\mathit{Ke}+W+C\right)^{4}$ on the denominator.  The
likelihood ratio for the hypothesis that collusive draws have the same
strength as regular draws isabout $e^{11.8}\simeq 10^5$: we conclude
that the high number of draws among the three accused players was
unlikely to be due to chance.

One plausible and charitable explanation for this result is that
Soviet players have a higher propensity to draw than western players.
We can investigate this by considering the interzonal tournament which
took place in Stockholm in 1962.

\subsection{Chess collusion: interzonal Stockholm 1962}
...
$\vdots$&&&&&\\
\end{tabular}
\caption{Subset of the Fifth Interzonal round-robin, Stockholm 1962.\label{interzonal}
  Results $\in\{0,\halfw,1\}$ and box colour refer to row
  player.  Thus, for example, Fischer played black against Geller and drew}
\end{table}
...
players draw, and is inactive whenever a game has at least one
non-Soviet player.  Then $H_N\colon D_s=D$ corresponds to no collusion
and the likelihood function for table~\ref{interzonal} would be
...
The maximum likelihood estimate is $D\simeq 0.043, D_s=0.354$; a
constrained maximization gives a likelihood ratio of about 5096, or a
$p$-value of about $3.6\times 10^{-5}$.  Indeed we could go further
and plot a profile likelihood curve for the log-odds ratio
$\ln\left(D_s/D\right)$ which, if Dirichlet, would be accurately
parabolic over a wide range~\cite[page 343]{ohagan2004}.  This is
illustrated in Figure~\ref{proflike}, that shows a credible interval
of about 1.8-2.4.
...
\caption{Profile likelihood \label{proflike} for odds ratio
  $\log\left(D_s/D\right)$: note the accurately parabolic figure in
  the region plotted}
  \end{center}
\end{figure}
...
In this context we note, following \cite{langer2013}, that a player
may well adopt a riskier strategy after a sequence of defeats,
adopting a ``nothing-to-lose'' mentality.  Such a phenomenon would be
detectable, at least in principle, by generalizing the personalised
draw likelihood function \ref{KKA_personalised_drawmonster} and
seeking evidence for the draw strength changing as a tournament
progresses: it would become stronger for a player on a winning streak,
and weaker for a losing player.
...
would admit such simulations and might be interesting to compare results.
