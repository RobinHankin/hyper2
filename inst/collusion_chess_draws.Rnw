...
considers~$n$ players and assigns non-negative number $p_i$ to player
...
competitions such as {\em MasterChef Australia}~\citep{hankin2017},
...
Here I show how Bradley-Terry may be generalized ot account for the
...
=
\prod_{i<j}
\left(\frac{D}{p_i+p_j+D}\right)^{D_{ij}^\mathrm{white}+D_{ij}^\mathrm{black}}
\end{equation}
