...
$p_1,\ldots,p_n,\draw$ (here \draw\ is the draw monster's strength,
fraktur font being used for reified entities' strengths) with $\draw+\sum
p_i=1$ and
...
\begin{equation}
\prod_{i<j}
\left(\frac{\draw}{p_i+p_j+\draw+\white}\right)^{D_{ij}^\mathrm{white}}
\left(\frac{\draw}{p_i+p_j+\draw+\white}\right)^{D_{ij}^\mathrm{black}}
=
\prod_{i<j}
\left(\frac{\draw}{p_i+p_j+\draw}\right)^{D_{ij}^\mathrm{white}+D_{ij}^\mathrm{black}}
\end{equation}
...
maximum likelihood estimate for draw strength $\hat{\draw}\simeq 0.53$:
...
$\white$, the extra strength conferred by playing white: it is conceivable
that $\white=0$ as this term does not appear on its own in the numerator of
any term.  We may test $H_0\colon \white=0$ using the Method of Support by
maximizing the likelihood subject to the constraint $\white=0$ and
...
drawn game is counted as a win for either of the draw monsters.  Under
...
monster with strength \cdraw, who wins when two colluding players
draw, and is inactive whenever a game has at least one non-colluding
player.  Then $H_N\colon\cdraw=\draw$ corresponds to no collusion and
the likelihood function would have
...
function for the interzonal has 297 terms, including $\draw^{110}$.  To
investigate the possibility of increased Soviet drawing proclivity, we
introduce a Soviet draw monster with strength~$\draw_s$, who wins when two
Soviet players draw, and is inactive if a game has at least one
non-Soviet player.  Then $H_N\colon \draw_s=\draw$ corresponds to no collusion
...
The maximum likelihood estimate is $\draw\simeq 0.043, \draw_s=0.354$; a
...
$\log\left(\draw_s/\draw\right)$ which, if Dirichlet, would be accurately
