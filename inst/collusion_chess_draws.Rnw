...
firstly, the white player is at a considerable
advantage~\cite{asdfasdffdsfds}; and secondly, many chess games end in
a draw: white wins about $37.5\%$ of all games, black wins $27.6\%$,
and about $39.4\%$ are drawn.  We consider the first-mover advantage
first, then consider the effect of drawn games.
...
debated with opinions ranging from Rowson who compares first-move
advantage to that of serving in tennis, to that of commentators such
as \citeauthor{adorjan} who claims that White's advantage is a
``delusion''.  \cite{asdfasdasfd} goes further, asserting that Black
can exploit the situation by mounting a directed response to White, of
the type seen in analysis of sequential zero-sum games, in which the
first-mover advantage can be positive or negative.
...
Because gradients are available, convergence is generally rapid and
hypothesis-specific constraints on parameter space are, in general,
...
in this case no draw monster appears alone anywhere in the likelihood
function.
...
order to conserve energy for subsequent matches~\citep{moul2009}.
...
One plausible explanation for the previ
...
parabolic over a wide range (OHAGAN AND FORSTER, p343).  This is
illustrated in Figure~\ref{DOESNOTEXIST}, that shows a credible
interval of about HAVENOTDONETHISET.
...
seeking evidence for the draw monster's strength changing as a
tournament progresses: it would become stronger for a player on a
winning streak, and weaker for a losing player. 

\cite{moul2009} present detailed Monte Carlo simulations to assess the likely impact of any Soviet cartel on tournament outcomes.  In principle, the present system of likelihood and reified entities would admit such simulations and might be interesting to compare results.
