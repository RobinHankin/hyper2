...
a new generalization of Bradley-Terry in which a chess match is
regarded as a three-way competition between the two players and an
entity that wins if the match is drawn.  Bradley-Terry can be further
...
advantage~\cite{asdfasdffdsfds}; and secondly, many chess matches end
in a draw.  Considering white advantage first, we find an overall mean
of about $(+0.375=0.394-0.276)$.  However, interpretations of this
statistic vary and the issue is hotly debated with opinions ranging
from Rowson who compares first-move advantage to that of serving in
tennis, to that of commentators such as \citeauthor{adorjan} who
claims that white's advantage is a ``delusion''; and that Black can
mount an intelligent and dynamic response to white: ``chess is a game
of complete information, and Black's information is always
greater---by one move!''.

Chess games may be won by either player, or may be drawn. The
implication of a draw is that neither player is better than the other.
Before 1867, tournament games that were drawn were replayed and in
this context we note that this procedure reduces a chess match to a
...
awarding each player of a drawn match half a point, rendering chess a
...
34\% of games are drawn ($\chi^2$, $p<3\times 10^{-6}$).
...
being able to exert some control over the probability of a draw.  They
suggest that risk-averse players may pursue less vigorous strategies,
trading expected payout for reduced variance. \cite{langer2013} {\bf
  cite something better here} point out that rational players in an
ongoing competitive situation modify their strategies depending on
whether they are leading or lagging: the lagging player will adopt
higher risk strategy (`nothing to lose'), while the leading player is
...
Such arguments suggest that considering a drawn chess match as a
qualitatively different outcome from a win for either player: the play
itself tends to be different, the drawing players have a different
mindset, and drawn games have less value in the wider context of the
game.
...
generalization of Bradley-Terry in which a chess match is regarded as
a three-way competition between the two players and an entity that
wins if the match is drawn.  The system is further modified to account
...
If we consider a chess match to be a three-way trial, we define a
...
representations of chess match results.  However, they differ markedly
from previous likelihood functions of draws in that they include
entities representing proclivity to draw $D$, and first-mover
advantage ($W$).


Considering white player a

The method of \citet{Edwards1972} involves only
numerical optimization (not integration which is computationally
expensive), and because gradients are available convergence is
generally rapid.
...
two-units-of-support per degree of freedom suggested by
\citeauthor{edwards1972}; we can convincingly reject the hypothesis that
$W=0$ and infer that playing white confers a significant advantage.
This is consistent with expectation, but it is now possible to go
...
MLE above, and considering a match between say Karpov and Anand, the
strengths are Karpov=0.064, Anand=0.151, white =0.104 and draw =0.53.
Thus we would estimate that Karpov playing white against Kasparov
would win with probability
...
probability $\simeq 0.174$ (compare $+0.075\,=0.627-0.297$ if Karpov
plays black).
...
region of its asymptotic distribution with a p-value of about 0.0072.
We can justifiably assert that the players do indeed have different
strengths, even if we account for white advantage and the proclivity
to draw.
...
total of $18+6=24$ matches playing white, $13+2=15$ games playing
...
\cite{moul2009}: the question of Soviet collusion during the period
when the USSR dominated the struggle for the world championship has
been one of the long-standing debates in the chess world. The
best-known (but not unique) criticism was aired by Bobby Fischer some
weeks after the 1962 Candidates Tournament in Curacao
...
Filip     &TCH     &             &NA      &NA     &NA     &NA     &NA     &NA     &A \\
\end{tabular}
\caption{Cura\c{c}ao 1962 candidates matches, world championship 1963}
\end{table}
