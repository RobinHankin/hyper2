...
\caption{Results \label{parkruntable} of simulated parkrun}
\end{centering}
\end{figure}
...
that $r$ is about 0.25 and $x$ is between about 0.4 and 0.8.  Recall
that, in this model, $x=0.5$ implies that the Plackett-Luce strength
of each competitor is twice that of the next; and if $x=0.25$, we can
expect Hamilton to beat than the second-best competitor with
probability $1-0.25=0.75$.  The resulting probability model [viz,
  Bradley Terry with strengths $\propto 0.75^i, i\in\left\lbrace
  1,\ldots,n\right\rbrace$; Hamilton with strength $\propto 0.75^5\sim
  0.237$] is succinct and powerful.  For example, using numerical
sampling with $10^5$ trials, we see that Hamilton's expected placing
is about 5.8.  Further, the number of points scored is an important
statistic describing competitors'
performance~\cite{hankin2023_formula1points}.  Using
$(25,18,15,12,10,8,6,4,2,1,1)$ [that is, 25 points for first place, 18
  for second, and so on] we can esitmate that Hamilton's expected
points would be about 9.6 per race, potentially valuable information.
