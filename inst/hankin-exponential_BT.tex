...
figure~\ref{parkrunlikelihood}.  From this, we see that $r$ is likely
...
$r=0.77, x=0.7$ for $10^5$ simulated parkruns with $57$ competitors.
...
(this time with $10^4$ trials per point), we obtain
figure~\ref{formula1}.  From this, we see that $s$ is about
0.25 and $x$ is between about 0.4 and 0.8.  Recall that, in this
model, $x=0.5$ implies that the Plackett-Luce strength of each
competitor is twice that of the next; and if $x=0.25$, we can expect
Hamilton to beat than the second-best competitor with probability
$1-0.25=0.75$.  The resulting probability model [viz, Bradley Terry
  with strengths $\propto 0.75^i, i\in\left\lbrace
  1,\ldots,n\right\rbrace$] is rich in inferential capability.  It
predicts, for example, the placing distribution for mid-range
competitors.  Taking $i=10$ as an example, 
