...
Above we see that the schedule was to have play on three consecutive
days followed by a one- or two- day break.
...
```{r testwm0, cache=TRUE}
specificp.gt.test(Hf, "wm", 0)
```

Above we see that $H_0\colon\mathtt{wm}=0$ may be rejected with a
$p$-value of about $3\times 10^{-11}$.
