...
With this model, whatever the value of $D$ and $\lambda$, there is a
pathology which BT cannot deal with.  Still not quite 100% sure but it
seems that the maximum likelihood estimate of the players' strengths
is zero except for Fisher and Petrosian; and indeed we see Fisher at
57% and Petrosian at 42% [with $\lambda = 1.1, D=1.888$].  This on the
grounds that these players never lost playing black (there are some
$2\times 3$ tables at the end).  This might be considered a defect of
