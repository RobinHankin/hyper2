...
Above, the intersection between the red and black lines is considered
in `hankin-exponential_BT.Rmd`.
...
```{r fourcompetitors_closeup}
worker_closeup <- function(...){

o <- 1:4

plot(c(-0.2,0),c(-1.4,-1.36), type='n',
     xlab="log x", ylab="log-likelihood", main="Focal competitor came third")
legend("topright",lty=1,col=c("black", "red", "blue", "green"),
       legend=c("r=1", "r=2", "r=3", "r=4"), bg="white")
...
points(log(x),log(sapply(x,f4)), type="l", col="green")
    ...
    ## Now find where the red line (r=2) intersects the green line
    ## (r=4) and also where the green line (r=4) intersects the blue
    ## line (r=3)
    
jj <- uniroot(f = function(x){sapply(x,f3) - sapply(x,f4)},
        interval = c(0.2,0.9999))
abline(v=log(jj$root),col='gray') # r=3 crosses r=4

jj <- uniroot(f = function(x){sapply(x,f2) - sapply(x,f4)},
        interval = c(0.2,0.9999))
abline(v=log(jj$root),col='gray') # r=2 crosses r=4
}

worker_closeup()
pdf(file="1234third_closeup.pdf")
worker_closeup()
dev.off()
```
...
```{r simulate57runners, cache=TRUE}
ax <- BT(57, 0.7) # strength of field
round(0.77 * 57)  # this is my identification
rrace(ax)
which(rrace(ax) == "p44")
which(rrace(ax) == "p44")
which(rrace(ax) == "p44")
which(rrace(ax) == "p44")
system.time(mypos100000 <- replicate(100000,which(rrace(ax) == "p44")))
```

```{r label=tableplotter}
tableplotter <- function(...){
    plot(table(mypos100000),
         xlab = "focal competitor rank", ylab = "frequency"
         )
}
tableplotter()
pdf(file = "parkruntable.pdf")
tableplotter()
dev.off()
sum(mypos100000 <= 38)
```
...
```{r readandcutout}
a <- read.table("formula1_2023.txt")
a <- a[,-ncol(a)]
rownames(a)
```
...
```{r simulatehamilton, cache=TRUE}
n <- 20 # number of racers
ax <- BT(n, 0.66) # strength of field
round(n * 0.23)  # this is Hamilton's identification
rrace(ax)
which(rrace(ax) == "p 5")
which(rrace(ax) == "p 5")
system.time(hampos100000 <- replicate(100000,which(rrace(ax) == "p 5")))
```

```{r}
plot(table(hampos100000))
mean(hampos100000)
points <- c(25,18,15,12,10,8,6,4,2,1,1,rep(0,100))
p <- points[hampos100000]
mean(p)
sd(p)
```
