...
P
n <- 400 # number of matches between a pair 
M <- 0.15  # home monster
...
```{r,label=maxpmonsteq1,cache=TRUE}
mH <- maxp(HP,n=3,give=T)
mH
```


```{r proflikesyntha,cache=TRUE}
home <- seq(from=0.12,to=0.16,len=15)
supp_lambda_eq_0 <- profsupp(HP,"home",home,relative=FALSE)
```

```{r plotsynthprof}
plot(home,supp_lambda_eq_0,type='b')
plot(home,supp_lambda_eq_0 - max(supp_lambda_eq_0),type='b')
abline(h=c(0,-2))


```{r marginalsynthcalct,cache=TRUE}
f <- function(lambda){
 return(maxp(ha3(P,lambda=lambda),n=1,give=TRUE)$value)
}

lam <- seq(from=0.5,to=1.3,len=7)
supp_M_eq_0 <- sapply(lam,f)
```
...
```{r plotbothtogether}
jj <- c(supp_M_eq_0,NA,supp_lambda_eq_0)
