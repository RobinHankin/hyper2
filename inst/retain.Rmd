---
title: "Illustrative use of retain()"
...
## Function keep() vs function retain()


Functions `keep()` and `retain()` can be confusing, here is a tiny
example that should clarify things.  
...
```{r}
ordertable2supp(a,misslast=FALSE)
```
...
`retain()`, which do slightly different things.  First, `keep()`:

```{r}
keep(ordertable2supp(a,misslast=FALSE),1:2)
```

Above, function `keep()` has effectively set `c=0` and then discarded
the `c^5` term.  This process, while natural, has the effect of
replacing `(a+c)^-4` with `a^-4`, which has no natural probabilistic
interpretation: we are ignoring `c`'s victories but nevertheless
interpreting joint strength term on the denominator [viz `a+c`] as a
*loss* for `a`.
...
strength whose wins we ignore.  Perhaps this has a natural
interpretation in probability, but if so I don't see it.
