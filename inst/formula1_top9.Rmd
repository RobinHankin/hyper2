...
o  <- apply(f2017a[,seq_len(20)],2,f)
rownames(o) <- rownames(f2017a)
(f2017a <- o)
```
...
details <- read.table("formula1_conditions.txt",header=TRUE)
alldrivers <- c("Ham_dry","Ham_wet",rownames(f2017a)[-1])
(L2017 <- hyper2(pnames=alldrivers)) # uninformative
for(i in seq_len(ncol(f2017a))){
	venue <- colnames(f2017a)[i]
	conditions <- details[which(details$venue == venue),4]
	o <- f2017a[,i]
	names(o) <- rownames(f2017a)
	if(conditions == "wet"){
...
H <- hyper2(pnames=c(rownames(f2017a),"S"))
noscore <- c("Ret", "WD", "DNS", "DSQ", "DNP", "NC")
