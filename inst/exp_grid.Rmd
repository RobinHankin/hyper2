...
for(i in seq_len(n)){
    ...
    B[i,] <-
        c(biasr(jj, 4), mser(jj, r_true = 4), biasx(jj, 0.7), msex(jj, x_true = 0.7))
}
B
```

```{r plotb}
colnames(B) <- c("bias(r)", "mse(r)", "bias(x)", "mse(x)")
plot(B[,c(1,3)])
plot(B[,c(1,2)])
plot(B[,c(3,4)])
```

```{r vary0_1, cache=TRUE}
n <- 10
B_1 <- matrix(NA, n, 4)
for(i in seq_len(n)){
    jj <-
        makemaxliketable(n=10000, nobs=10, no_of_competitors=9, allX=allX9, r_true = 4, x_true = 0.7)
    B_1[i,] <-
        c(biasr(jj, 4), mser(jj, r_true = 4), biasx(jj, 0.7), msex(jj, x_true = 0.7))
}
B_1
...
```{r plotba}
colnames(B_1) <- c("bias(r)", "mse(r)", "bias(x)", "mse(x)")
colMeans(B_1)
plot(B_1[,c(1,3)])
plot(B_1[,c(1,2)])
plot(B_1[,c(3,4)])
...
Above we have that the bias is about -0.195 with a standard error 
