...
different from, the analysis in `eurovision.R`.
...
admissible region with $p_\mathrm{Hamilton}\approxequal\frac{1}{25}$).
...
```{r,L4like,cache=TRUE}
small <- 0.01
L4 <- maxp(formula1, startp=c(small, rep((1-small)/24,23)),fcm=rbind(c(-1,1,rep(0,22))),fcv=0,give=TRUE)
L4
```
...
```
(the p-value seems to be too low to be believable).
...
p <- seq(from=0.5,to=0.96,len=20)
pnames(HP)
u <- profsupp(HP,"Ricciardo",p,n=1)
u
plot(p,u-max(u))
```
