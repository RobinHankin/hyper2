...
```{r}
jj <- outer(names(m),names(dp),"==")
ox <- dp
oy <- m[which(jj,arr.ind=TRUE)[,1]]
```
...
From the above, we see that the first four drivers (Hamilton, Vettel,
Raikkonen, Bottas) are respectively first, second, third and fourth
according to both ranking procedures.  The lower-ranked players,
having estimated strengths of less than 0.03, are more scattered as
one might expect.
