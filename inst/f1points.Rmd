...
abstract: >
  ...
  itself is intrinsically arbitrary.  Here, I apply generalized
  Plackett-Luce likelihood functions to Formula 1 motor racing, and
  use maximum likelihood techniques to identify a ranking for the competitors;
  this method includes no use of an arbitrary points system.
  Using this methodology, I assess a number of reasonable points
  scoring systems objectively.  In a well-defined statistical sense, a
  ``winner-takes-all'' points system outperforms the other points
  systems considered.
keywords: 
...
as the maximal value of $n$ such that places $1,2,\ldots n$ all match.
...
```{r,label=likevspointszipf,cache=TRUE,echo=FALSE,fig.cap="As for figure \ref{orderingbypointsandlikelihood} \\label{orderingbypointsandlikelihood_zipf} but points ranking calculated by Zipf's law.  Note the generally poorer agreement between the two ranking systems ALSO R PLOTTING SUCKS HERE AND I WILL HAVE TO CHANGE THE PDF BY HAND WITH INKSCAPE TO MAKE IT LOOK GOOD"}
par(mfrow=c(2,2))
par(mar = c(3,3,3,3))
...
axis(2,at=1:4)
points(s2016[wanted,1],pch=1,type='b')
points(s2017[wanted,1],pch=2,type='b')
points(s2018[wanted,1],pch=3,type='b')
points(s2019[wanted,1],pch=4,type='b')
#legend("topright",lty=0,pch=1:4,legend=c("2016","2017","2018","2019"))
par(xpd=TRUE)
text(x=6,y=s2016[wanted[6],1],pos=4,"2016")
text(x=6,y=s2017[wanted[6],1],pos=4,"2017")
text(x=6,y=s2018[wanted[6],1],pos=4,"2018")
text(x=6,y=s2019[wanted[6],1],pos=4,"2019")
```
...
axis(1,at=1:6,labels=names(formula1_points_systems(top)[wanted]))
axis(2)
points(s2016[wanted,2],pch=1,type='b')
points(s2017[wanted,2],pch=2,type='b')
points(s2018[wanted,2],pch=3,type='b')
points(s2019[wanted,2],pch=4,type='b')
#legend(x=3.4,y=0.67,lty=0,pch=1:4,legend=c("2016","2017","2018","2019"))
par(xpd=TRUE)
text(x=6,y=s2016[wanted[6],2],pos=4,"2016")
text(x=6,y=s2017[wanted[6],2],pos=4,"2017")
text(x=6,y=s2018[wanted[6],2],pos=4,"2018")
text(x=6,y=s2019[wanted[6],2],pos=4,"2019")
```
...
axis(1,at=1:6,labels=names(formula1_points_systems(top)[wanted]))
axis(2)
points(s2016[wanted,3],pch=1,type='b')
points(s2017[wanted,3],pch=2,type='b')
points(s2018[wanted,3],pch=3,type='b')
points(s2019[wanted,3],pch=4,type='b')
#legend("topleft",lty=0,pch=1:4,legend=c("2016","2017","2018","2019"))
par(xpd=TRUE)
text(x=5,y=s2016[wanted[5],3],pos=3,"2016")
text(x=5,y=s2017[wanted[5],3]-0.01,pos=1,"2017")
text(x=5,y=s2018[wanted[5],3],pos=3,"2018")
text(x=5,y=s2019[wanted[5],3],pos=3,"2019")
```
...
respectively.  These figures differ in detail but all have one
feature: the winner-takes-all points system consistently outperforms
the other systems.  This is particularly evident in figure
\ref{simulated_complete_agreement}.


# Discussion

Many competitive situations involve ranking competitors and one common
method for combining repeated observations is to assign points for the
winner, second place, third place, etc.  The overall ranking is
decided on the basis of accumulated points.  However, statistical
analysis made difficult by the arbitrary values of the points given.

The points system used in Formula 1 motor racing is a source of lively
debate from many perspectives, with changes being controversial.  
Here I treat total points scored as a random variable and compare
different point allocation schemes against objective Plackett-Luce
ranks.  By several criteria, a ``winner takes all'' performs better
than other reasonable points systems.  However, such a system
furnishes little incentive for mid-ranking competitors and as such
might result in duller races.
