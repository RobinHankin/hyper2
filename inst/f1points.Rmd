...
```{r,label=montecarlosetup2, cache=TRUE}
f2017 <- read.table("formula1_2017.txt",header=TRUE)[seq_len(top),1:20]
m <- maxp(ordertable2supp(as.ordertable(f2017)))
```

```{r,label=montecarlosetup3}
points_real
...
```{r,label=montecarlorealpointsdoer,cache=TRUE}
or <- o_real(1000)
```

Now some more, top n:

```{r,label=defineotop, cache = TRUE}
o_top <- function(top,n=100){replicate(n,resampling(m,f2017,points=rep(1,top)))}
```


```{r,label=executeotop1, cache = TRUE}
ot1 <- o_top(1,n=1000)
```

```{r,label=executeotop2, cache = TRUE}
ot2 <- o_top(2,n=1000)
```

```{r,label=executeotop3, cache = TRUE}
ot3 <- o_top(3,n=1000)
```

```{r,label=executeotop4, cache = TRUE}
ot4 <- o_top(4,n=1000)
```

```{r,label=executeotop5, cache = TRUE}
ot5 <- o_top(5,n=1000)
```

now some other schemes:

```{r,label=defineotherschemes}
...
```{r,label=ozcalculator,cache=TRUE}
oz <- o_zip(1000)
```

```{r,label=oecalculator,cache=TRUE}
oe <- o_exp(0.5,1000)
```
...
dimnames(M) <- list(system=c("real",paste("top",seq_len(5)),"oz","oe"),correct=0:11)
M
```
