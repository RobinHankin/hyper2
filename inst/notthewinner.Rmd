...
space.  The first step is to define a suitable likelihood function for
our data.

What we will do is combine ${\mathcal O}$ with other, perhaps more
...
(the numerical value of `small` is immaterial).
...
We can present a support function for $H_0,H_1,H_2$:
...
Now we make another observation ${\mathcal O}$ that is weakly
informative: that person `1` did not win:
...
So `SLa` is a compound object that gives a likelihood function for the
joint observation of the order statistics and ${\mathcal O}$.  We can
find the MLE for different observations:
