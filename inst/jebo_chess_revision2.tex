...
\textcolor{DarkGreen}{The Bradley-Terry techniques used here furnish
  evidence that collusive games played during during Cura\c{c}ao 1962
  were less detrimental to future performance than noncollusive games.
  However, there was no cognitively realistic model for mental fatigue
  (for example, downweighting heavy workloads long in the past) and
  the criterion for restedness itself was quite coarse. not allowing
  It would be possible in principle to investigate the effect of
  recovery using similar reified entities.}
