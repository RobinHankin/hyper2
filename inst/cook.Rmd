...
the _loser_ (which is me).  We have two hypotheses

* $H_1: \mbox{I might very well be the worst chef in the family}$
* $H_2: \mbox{I am not the worst chef in the family}$

Observe that $H_2$ implies that someone is worse than me and this
corresponds to a restricted parameter space.  The first step is to
define a suitable likelihood function for our observation:
...
In the above, `L` is a list of hyper2 objects corresponding to a
possible order statistic (we see the first two).  Any order is
permissible, so long as player `a` (me) is bottom.  Now we maximize
the likelihood under $H_1$, that is, perform an unconstrained
maximization:
...
The likelihood at the evaluate is 1 (technically, this is wrong
because likelihood is defined only up to a multiplicative constant.  A
correct statement is: the likelihood function evaluated by
...
at least one other person, here chosen to be person `b`:
...
We can see that the likelihood ratio ${\mathcal L}(H_2)/{\mathcal
L}(H_1)$ is 0.5 for an observation that I come last.  This is exactly
the same as the likelihood ratio for the following situation: we toss
a coin which is either fair ($H_\mbox{fair}$) or two-headed
