...
We may apply `hyper3` idiom to the dataset.  First we show
`home_away3()` being used with a specific value for $\lambda$:

```{r, label=hyper3_homeaway,cache=TRUE}
(o <- home_games_won + 1i*away_games_won)
f <- function(l){ maxp(home_away3(o,l),give=TRUE,n=1)$likes}
l <- exp(seq(from=log(2),to=log(6),len=10))
L <- sapply(l,f)
L <- L - max(L)
```

```{r,label=plothyper3home}
plot(l,L,type="b")
abline(h=c(0,-2))
plot(log(l),L,type="b")
abline(h=c(0,-2))
...
l <- exp(seq(from=log(0.8),to=log(1.9),len=14))
L <- sapply(l,f)
L <- L - max(L)
```
...
```{r,label=optimizebasebasll,cache=TRUE}
o <- optimize(f,c(0.2,0.4),maximum=TRUE)
o
```
