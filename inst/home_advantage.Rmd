...
```{r sfasdf, cache=TRUE}
RCLF3_lambda_max <- o$maximum
RCLF3_max <- maxp(home_away3(home_away_table(a,give=FALSE),RCLF3_lambda_max),give=FALSE)
round(RCLF3_max * 1000)
```
...
for(hometeam in teams){
    for(awayteam in teams){
        ...
        h3_exp[hometeam,awayteam] <- (
            n_matches[hometeam,awayteam] * (lambda* RCLF3_max[hometeam])/
            (lambda* RCLF3_max[hometeam] + RCLF3_max[awayteam])
        )

        h3_exp[hometeam,awayteam] %<>%
            add(1i*
                n_matches[hometeam,awayteam] * (RCLF3_max[awayteam])/
                (lambda* RCLF3_max[hometeam] + RCLF3_max[awayteam])
        )
    }
}
...
round(expectation3(RCLF3_max[c("Rangers","Celtic","Falkirk","Livingston")],aF,RCLF3_lambda_max),1)
```
...
save(RCLF3_table,  RCLF3, RCLF3_max, RCLF3_lambda_max, file="RCLF.rda")
```
