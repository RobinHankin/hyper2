...
\Abstract{
  The \pkg{hyper2} package provides functionality to work with
  ...
  natural \proglang{R}~idiom to manipulate such likelihood functions.  Here, I
...
The \pkg{hyper2} package \citep{hankin2009,hankin2017} furnishes
...
However, the majority of software is written in the \proglang{R}~computer language \citep{rcore2023}, which includes extensive
...
considers pairwise comparisons using \code{glm} but cannot deal
...
\end{verbatim}
%
show how the `\code{map}' class of the Standard Template Library is
used with `\code{weightedplayervector}' objects mapping strings to
long doubles (specifically, mapping player names to their
multiplicities), and objects of class `\code{hyper3}' are maps from
a `\code{weightedplayervector}' object to long doubles.  One
...
is provided in the package \pkg{hyper2}~\citep{hankin2017,hankin2009},
...
\proglang{S}3 class `\code{hyper3}' for this type of object;
extraction and replacement methods use \pkg{disordR} discipline
...
The original motivation for \code{hyper3} was the analysis of
...
\begin{figure}[t!]
\centering
\includegraphics[trim = 0 5 0 20,clip]{jss_hyper3-figaba}
...
\begin{figure}[t!]
\centering
\includegraphics[trim = 0 5 0 20,clip]{jss_hyper3-figabbabb}
...
\begin{figure}[t!]
\centering
\includegraphics[trim = 0 5 0 40,clip]{jss_hyper3-plotwilcoxlike}
...
\begin{figure}[t!]
\centering
\includegraphics[trim = 0 5 0 40,clip]{jss_hyper3-testthejav}
...
\begin{figure}[t!]
\centering
\includegraphics[trim = 0 5 0 20,clip]{jss_hyper3-plottheloglikcont}
...
performance.  The following table, included in the \pkg{hyper2}
...
back-end making use of the \proglang{STL} `\code{map}' class which offers
