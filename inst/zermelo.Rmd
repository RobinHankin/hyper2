...
```{r funwithzermelo}
o <- zermelo(M, maxit=1000, give=TRUE)
dim(o)
head(o)
```

Above we see rows of `o` being successive approximations to the
maximum likelihood estimate for the BT strengths of the players.  The
process stops after 149 rows.  Looking at the bottom of the matrix
gives us the evaluate:
...
Above we see how each estimate is a convergent sequence.
Alternatively:
...
Above we see each multivariate estimate as a line.  The horizontal
line is the starting point (the top line of the matrix).  We may see
the performance of successive estimates:
...
The graph above would suggest that stopping after about 75 iterations
would be sufficient.
