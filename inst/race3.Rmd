...
allvenues <- function(h){
  H3 <- hyper3()
  cons <- constructor_2021_table[,1]
  for(i in seq_len(ncol(constructor_2021_table)-1)){
...
```{r,helpseqsupport,cache=TRUE}
o <- function(h){maxp(allvenues(h),give=TRUE,n=1)$likes}
l <- seq(from=0.5,to=4,len=6)
L <- sapply(l,o)
```
...
o <- maxp(allvenues(4),give=TRUE,n=1,hessian=TRUE)
```

```{r,analyse4}
pie(o$par)
eigen(o$hessian)$values
```
