...
At the evaluate of $\hat{a}=\sqrt{2}-1$ we can show that
...
as a standard support function which has a maximum value of zero.
Further, we can use either analytical or numerical methods to find a
credible interval.  With an $n$-unit of support criterion the
analytical solution to $\mathcal{S}(p)=-2$ is given by defining
$X=\log(3-2\sqrt{2})-n$ and solving $p(1-p)/(1+p)=X, or
$\left(1-X\pm\sqrt{1+4X+X^2}\right)/2$, the two roots being the lower
and upper limits of the credible interval.  Graphically:
