---
title: "The `rego` dataset"
author: "R. K. S. Hankin"
output: bookdown::html_document2
bibliography: hyper2.bib
---

```{r setup, include=FALSE}
set.seed(0)
knitr::opts_chunk$set(echo = TRUE)
library("hyper2")
library("magrittr")
options("digits" = 5)
options("width" = 100)
```

```{r out.width='20%', out.extra='style="float:right; padding:10px"',echo=FALSE}
knitr::include_graphics(system.file("help/figures/hyper2.png", package = "hyper2"))
```

To cite the `hyper2` package in publications, please use
@hankin2017_rmd.  Here we consider the `rego` dataset drawn from the
European Union, as analysed in the `rSRD` package [@staudacher2026]:

```{r readiconstable}
jj  <- as.matrix(read.table("rego.txt",header=TRUE))[1:16,]
jj[1:4, 1:6]
```

[Row 17 refers to summary statistics defined by Sziklai et al, not
used here].  Above each row is an issue and each column an author.
Entries are the number of times each issue is mentioned by each
author.

```{r putinorder, cache=TRUE}
rego_table <- as.ordertable(apply(jj, 2, rank, ties.method="first"))
rego_table[1:4,1:6]
rego <- suppfun(rego_table)
rego_maxp <- maxp(rego)
```

```{r showit}
rego_maxp
plot(rego_maxp)
plot(log(rego_maxp))
plot(sort(log(rego_maxp)))
pie(rego_maxp)
dotchart(rego_maxp, pch=16)
dotchart(log(rego_maxp), pch=16)
```


OK now, conditional on `rego_maxp`, I will calculate the probability
[actually, log-likelihood] of observing each of the judges' order
statistics:

```{r defcol, cache=TRUE}
getcol <- function(i){
  ordervec2supp(setNames(c(rego_table[,i]),rownames(rego_table)))
}
getcol(1)
loglik(rego_maxp, getcol(1))
loglik(rego_maxp, getcol(2))
loglik(rego_maxp, getcol(3))
```  

We can see that the probability [actually, log-likelihood] of
obtaining the observations of Botenga, Bompard, and Ernst are slightly
different from one another.  We can calculate this for all the
politicians in one vectorized expression:

```{r manydefcol, cache=TRUE}
L <- sapply(seq_len(ncol(rego_table)),
    function(i){loglik(rego_maxp, getcol(i))}
    )
```


And plot them:

```{r plotmanydefcol}
L <- L - max(L)
plot(L, pch=16)
grid()
n <- ncol(rego_table)
mypos <- rep(2,n)
mypos[1] <- 4
text(seq_len(n), L, colnames(rego_table), pos=mypos, col="gray")
```

Above we see that Kaljurand is a bit of an outlier.  Conditional on
the evaluate, his is (by far) the least likely order statistic.

We may go slightly further and use the _other_ politicians:

```{r defineothers}
fothers <- function(i){ # column i
  maxp_others <- maxp(suppfun(rego_table[,-i]))
  return(loglik(maxp_others, getcol(i)))
} 


```{r useothers, cache=TRUE}
Lothers <- sapply(seq_len(ncol(rego_table)), fothers)
```

```{r}
plot(Lothers)
```


### Package dataset {-}

Following lines create `rego.rda`, residing in the `data/` directory
of the package.

```{r, label=saveregodataset}
save(rego, rego_maxp, rego_table, file="rego.rda")
```

## References {-}
