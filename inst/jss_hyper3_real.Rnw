...
chess~\citep{hankin2020_jss}.  Note that $\lambda < 1$ would correspond to
...
$p_i\geqslant 0, \sum p_j=1$, would be proportional to
%
\begin{equation}\label{mostgeneral}
  ...
  \frac{
    ...
    \prod_{j\in S_i\cup\overline{S_i}}\lambda_{ij}p_j
  }
  \right)^{w_i}
\end{equation}
%
where playing set $S_i$ is a subset of the competitors $\left\lbrace
1, \ldots, n\right\rbrace$ and the $\lambda_{ij}\geqslant 0$ are
multiplicative factors that account for ephemeral or temporary changes
in competitive ability.  The data $w_1,\ldots w_N$ correspond to the
number of ``wins'' for playing set $S_i$ as per $w_{12},w_{34}$ in
...
the reified entity probability for a draw is $D/(2p+D)$: the
probability of a draw is a function of both $D$ and $p$; clearly
undesirable.  Compare Table~\ref{windrawloss}, in which the
probability of a draw is $\frac{2D}{2+2D}$; the odds of a draw is
simply $D$, regardless of common team strength $p$.

\paragraph{Won games only}
Again with $p_i=p_j=p$, now considering only won (that is, not
score-drawn) games, the probability of a home win is
$\frac{\lambda}{1+\lambda}$: the odds of a home win is simply
$\lambda$, independently of the Bradley-Terry strength.

\paragraph{One dominating team} Now consider the case where $p_j=0$.
The old probability of drawing was $\frac{D}{D+p_i}$, again not
independent of the teams's strengths.  Compare Table~\ref{windrawloss}
in which the draw probability is $\frac{\lambda}{\lambda+D}$ for a
home draw.
...
\caption{Probabilities \label{windrawloss} of a win, draw, loss for
  two teams under a multiplicative Bradley-Terry model.  Observe that
  ...
  a home ground advantage of $\lambda$; and $D$, a propensity to draw}
\end{table}
\begin{table}[t!]
\centering
\begin{tabular}{l|c|c}\\&
...
{$i$ plays away} \\
  \hline
  $\Prob{(\mbox{$i$ wins})}$                             &
  $\frac{H+p_i}{p_i + p_j + H + D}$  &
  $\frac{  XXXXXXXXXXXXXXXXXXXXXXXXXxxxxp_i}{p_i + p_j + H + D}$  &
  $\Prob{(\mbox{draw})}$                                 &
  $\frac{D(p_i + p_j)}{\lambda p_i+D(p_i+p_j) + p_j}$     &
  $\frac{D(p_i + p_j)}{p_i + D(p_i+p_j) + \lambda p_j}$   \\
  $\Prob{(\mbox{$i$ loses})}$                            &
  ...
  $\frac{\lambda p_j}{p_i + D(p_i + p_j) + \lambda p_j}$  \\
\end{tabular}
\caption{Probabilities \label{windrawloss_reified} of a win, draw, loss for
  two teams under a reified Bradley-Terry model.  Observe that XXXXXXXXXX}
\end{table}
%
