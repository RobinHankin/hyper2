...
wide uptake in the {R} community.  The canonical case is to
consider~$n$ players who compete against one another in pairs; the
basic inference problem is to estimate
...
approach of~\citet{hankin2020_jss}.  Firstly, if we consider two players
...
Plackett-Luce likelihoods~\citep{plackett1975_jss} likelihood function for
rank data.
...
\pkg{hyper2} package includes function \fct{cheering} which
