...
and thus $p_{C}=p_{BB}=p_{RB}$.

However, any realistic choice model will have $\Prob(C | \left\lbrace
...
suggests using a generalized extreme-value model.

Here I suggest an alternative in the form of a one-parameter
generalization of Plackett-Luce likelihood functions for ranking
options, using the multiplicative generalization to Bradley-Terry
strengths presented above.  As a synthetic example, we specify options
RB and BB for red bus and blue bus together with options C, T and W
for car, train, and walking.
