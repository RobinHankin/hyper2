...
\Abstract{
  ...
  Generally, ordinary Bradley-Terry is recovered if $\lambda=1$.  This
  approach offers multiple advantages over other generalizations such
  as reified Bradley-Terry.  It becomes possible to represent
  non-independence of successive observations in Plackett-Luce
  likelihood functions which allows a parametrization of the ``red
  bus, blue bus'' problem in preference analysis.  I present examples
  drawn from the fields of food preference and the esport {\em
    Counterstrike}.
}
...
chess~\citep{hankin2020_jss,hankin2020}.  Note that $\lambda < 1$
...
this paper I generalize the \pkg{hyper2} R
package~\citep{hankin2017_rnw_jss}, available on
CRAN~\citep{rcore2025}, for Bradley-Terry likelihoods to accommodate
this class of multiplicative factors with new \proglang{S3} class
\class{hyper3} and associated methods.
...
$w_1,\ldots w_N$ correspond to the number of ``wins'' for playing set
$S_i$ as per $w_{12},w_{34}$ in Equation~\ref{like1234}.  In practice,
...
Creating a likelihood function for this dataset is straightforward.
As an illustration of package idiom, we will consider the restriction
of equation~\ref{firstfewRCFL} to just Rangers vs Celtic matches, and
ignore score draws:
...
likelihood curve (Figure~\ref{profilesupportlambda}), or to find the
...
\pkg{hyper2} formalism may be used to assess this.  Consider the
likelihood function described in Table~\ref{windrawloss}.  This
presents a modification of the reified entity technique
of~\citet{hankin2020_jss} in which a drawn match was considered to be
won by a fictitious entity (a ``draw monster'') who possessed his own
BT strength.  However, the likelihood function of
Table~\ref{windrawloss} has a number of desirable features not present
in the reified entity parametrization.  Specifically,
Table~\ref{windrawloss} relating to players of identical strengths,
and the
...
\paragraph{One dominating team} Now consider the case where $p_i=p, p_j=0$.
The multiplicative model gives the probability of a home draw as
$\frac{D}{\lambda + D}$ and an away draw as $\frac{D}{1+D}$; compare
the reified model's $\frac{\mathfrak{D}}{p +
...
\caption{Probabilities \label{windrawloss} of a win, draw, loss for
  ...
  the reified Bradley-Terry model of~\citet{hankin2020}.  Observe that
...
$D$.  We are now in a position to consider the entirety of
Table~\ref{oldfirm}.  Bespoke package function \fct{home\_draw\_away3}
...
Placket-Luce to admit non-independence of successive arrivals can be
...
strength.  The \pkg{hyper2} R package implements the resulting
likelihood functions using \proglang{S3} class {\tt hyper3}, and
furnishes a wide range of functionality geared towards assessing
