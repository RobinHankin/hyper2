...
make_sushi_H <- function(a, lambda){
    ...
    for(i in seq_len(nrow(a))){
        H <- H + cheering3(sushi_types[a[i,]], e=ec, h=c(1, lambda))
    }
    return(H)
}
@
...
wanted <- sample(seq_len(nrow(a)), 5, replace=TRUE, prob=a[,1])
a <- a[wanted, -1]
@ 
...
H1888 <- make_sushi_H(a, strength=1.888)
H1000 <- make_sushi_H(a, strength=1.000)
@
