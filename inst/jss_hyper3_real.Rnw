...
The most general likelihood function would be
proportional to
%
...
As an illustration of package idiom, one way of representing a
likelihood function for Rangers-Celtic matches (here, ignoring draws)
would be:
%
...
\begin{figure}[htbp]
  \begin{center}
<<plotproflikeoldfirm, echo=FALSE, fig = TRUE>>=
...
\begin{figure}[htbp]
  \begin{center}
<<docontourplot, cache = FALSE, fig = TRUE>>=
...
$\Sexpr{round(pchisq(2*sushisupport,df=1,lower.tail=FALSE),3)}$.  We can
...
\begin{figure}[htbp]
  \begin{center}
<<plotlikesushi, echo = FALSE, fig = TRUE>>=
...
\begin{figure}[htbp]
  \begin{center}
<<csgo_plotlikelam, echo = FALSE, fig = TRUE>>=
