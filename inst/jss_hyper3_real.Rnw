...
$\frac{p_1+p_2}{p_1+p_2+p_3+p_4}$~\citep{hankin2010}.  It is natural to
combine this idea with that of a multiplicative factor.  For example,
suppose $p_1$ has a special move against $p_3$, and $p_4$ has just
been given a performance-enhancing drug, we might have a likelihood
functions proportional to $\frac{\lambda p_1+ p_2}{\lambda
  p_1+p_2+p_3+\mu p_4}$.  Then the effectiveness of $p_3$'s special
move and the enhancement afforded by the drug are measured by
$\lambda$ and $\mu$ respectively with values $>1$ corresponding to a
positive effect.
...
in~\cite{hankin2024_hyper3} in which the multiplicative factors are
known non-negative integers.
...
\cite[p153]{alvo2014}.  
by \cite{tverskey1972}.
The canonical problem is as follows. A traveller has a choice of going
...
which might incorporate a random component.  If considering whether to
use a car or a bus, the colour of the bus is of no importance to him.
Such preferences are problematic for Bradley-Terry: If our subject is
indifferent between cars and buses, say, $p_{C} = p_{RB}$ and also
indifferent to colour of bus, so $p_{RB}=p_{BB}$, the following
...
$\Prob(C| \left\lbrace C,RB\right\rbrace)=\frac{1}{3}$.  Various
solutions to this problem have been proposed.  \cite{tverskey1972},
for example, presents an approach involving overlapping aspects; 
...
Easily satisfying \citeauthor{edwards1992}'s two-units-of-support
criterion; alternatively, we could observe that this observation lies
in the tail region of its asymptotic $\chi^2_1$ distribution (Wilks's
theorem), with a $p$-value of \Sexpr{signif(pchisq(suppdiff*2, df=1,
  lower.tail=FALSE),3)}.
...
use these datapoint to define a bespoke likelihood function, using
...
s <- exp(seq(from = log(0.9), to = log(10), len = 12))
L <- sapply(s,f)
...
\caption{A\label{profilesupportsushi} profile support function for the
  exchangeability term $\lambda$.  Vertical gray line shows
  $H_0\colon\lambda =1$, horizontal lines show the
  two-units-of-support limits}
\end{center}
\end{figure}
...
viewership exceeded $1.3\times 10^6$, the highest in CS:GO history at
the time.  I study the performance of tournament champions {\em
  Cloud9} who, as underdogs, narrowly defeated aggressive European
...
comparable in skill [we fail to reject $H_0\colon p_{\mbox{\tiny\sc
      Skadoodle}}\neq p_{\mbox{\tiny\sc Stewie2k}}$, Plackett-Luce,
...
<<findopt, cache=TRUE, echo=FALSE>>=
optlam <- optimize(f,c(0.09, 0.14), maximum = TRUE)
evaluate <- optlam$maximum
support <- optlam$objective - f(1)
pvalue <- pchisq(support,df=1,lower.tail=FALSE)
@

<<calcklikelam, cache=TRUE, echo=FALSE>>=
lam <- exp(seq(from = -4,to = log(1.1), len = 23))
L <- sapply(lam, f)
@ 

<<findinterval, cache=TRUE, echo=FALSE>>=
...
\caption{A\label{profsupcounterstrike} profile support function for
  ...
  $\Sexpr{round(support,2)}$ relative to the evaluate
  $\hat{\lambda}\simeq\Sexpr{round(evaluate,3)}$ (small red bar),
  corresponding to a $p$-value of about $\Sexpr{round(pvalue,3)}$.
  Horizontal lines show the two-units-of-support credible interval of
  $(\Sexpr{round(lowerlimit,3)},\Sexpr{round(upperlimit,3)})$, marked
  in blue}
\end{center}
\end{figure}
...
a likelihood ratio of about $\Sexpr{round(support,2)}$.  We observe
that this exceeds the two units of support criterion of
\cite{edwards1992} [asymptotic $p$-value
  $\simeq\Sexpr{round(pvalue,3)}$] so we may infer that there is
