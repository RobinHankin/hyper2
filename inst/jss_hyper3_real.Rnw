...
package~\citep{hankin2017_rnw_jss}, available on CRAN~\citep{rcore2025},
for Bradley-Terry likelihoods to accommodate this class of
multiplicative factors with new \proglang{S3} \class{hyper3} and
associated methods.
...
for the same phenomenon by \cite{tverskey1972_jss}.  The canonical problem
...
RB and BB for red bus and blue bus together options C, T and W for
car, train, and walking.
...
$\Sexpr{round(pchisq(sushisupport,df=1,lower.tail=FALSE),3)}$.  We can
