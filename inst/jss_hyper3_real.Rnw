...
$p_1$'s special move, and $\mu$ the enhancement afforded to $p_4$ by
the drug; values $>1$ would correspond to a positive effect.  In this
paper I generalize the {\tt hyper2} R package~\citep{hankin2017_rnw},
available on CRAN~\citep{rcore2025}, for Bradley-Terry likelihoods to
accommodate this class of multiplicative factors.  The most general
likelihood function would be proportional to
