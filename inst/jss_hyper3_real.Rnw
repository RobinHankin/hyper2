...
the reified entity probability for a draw is $D/(2p+D)$, a function of
both $D$ and $p$; clearly undesirable.  Compare the multiplicative
formulation, in which the probability of a draw is $\frac{2D}{2+2D}$;
the odds of a draw is simply $D$, regardless of common team strength
$p$.
...
$\frac{p+H}{2p+H}$, making interpretation of $H$ problematic.

\paragraph{One dominating team} Now consider the case where $p_i=p, p_j=0$.
The multiplicative model gives the probability of a draw as
$\frac{D}{\lambda + D}$; compare the reified model's $\frac{D}{D+p}$,
again not independent of the team's strengths.
...
{$i$ plays away} \\
  ...
  $\frac{p_i+H}{p_i+p_j+D+H}$                             &
  $\frac{p_i  }{p_i+p_j+D+H}$                             \\
  $\Prob{(\mbox{draw})}$                                  &
  ...
  $\frac{p_j  }{p_i+p_j+D+H}$                             &
  $\frac{p_j+H}{p_i+p_j+D+H}$                             \\
\end{tabular}
\caption{Probabilities \label{windrawloss} of a win, draw, loss for
  ...
  for the two teams in isolation [considering non-drawn matches only];
  a home ground advantage of $\lambda$ or $H$; and $D$, a propensity
  to draw}
\end{table}
%
We are now in a position to consider the entirety of
Table~\ref{oldfirm}.  Bespoke package function
\fct{home\_draw\_away3} converts data in the form of
