...
\author{Robin K. S. Hankin~\orcidlink{0000-0001-5982-0415}\\University of Stirling}
...
\Abstract{
  ...
  A generalization of the Bradley-Terry model is presented in which
  strengths may be modified using a multiplicative factor $\lambda$;
  generally, ordinary Bradley-Terry is recovered if $\lambda=1$.  The
  $\lambda$ factor is well-suited to represent temporary or ephemeral
  modifications to Bradley-Terry strength such as the home-ground
  advantage, or the effects of fatigue.  The multiplicative approach
  offers several advantages over reified Bradley-Terry, which uses
  additive terms (``monsters'') to represent enhanced strengths.
  Computational support for this approach is given in new
  functionality of the established \pkg{hyper2} \proglang{R} package:
  multiplicative factors are handled in a bespoke \proglang{S3} class
  \class{hyper3}.  I present numerical methods that make
  likelihood-based inferences about the $\lambda$.  Multiplicative
  factors admit non-independence of successive observations in
  Plackett-Luce likelihood functions, which allows a parametrization
  of the ``red bus, blue bus'' problem in preference analysis.  I
  present examples drawn from the fields of food preference and the
  esport {\em Counterstrike}.  }
...
\Keywords{Plackett-Luce, Bradley-Terry, \proglang{R}}
\Plainkeywords{Plackett-Luce, Bradley-Terry, R}
...
wide uptake in the {R} community [see, for example,
\citet{turner2012,turner2020} and references therein].  The canonical
case is to consider~$n$ players who compete against one another in
pairs; the basic inference problem is to estimate
numbers~$\left({p_1,\ldots,p_n}\right)$, $p_i\geqslant 0$, $\sum
p_i=1$ so that $\Prob(\mbox{$i$~beats~$j$})=\frac{p_i}{p_i+p_j}$.
Information about the~$p_i$ may be obtained from the results of paired
comparisons between the players.

One generalization of the Bradley-Terry model is due
to~\citet{luce1959_jss}, in which the probability of competitor~$i$
...
chess~\citep{hankin2020_jss}.  Note that $\lambda < 1$
would correspond to a decrease in competitive ability.

Further, one might follow~\cite{hankin2017_rnw_jss} and consider teams of
...
example, suppose $p_1$ has just learned a new special technique, and
$p_4$ has just been given a performance-enhancing drug.  If so, we
might have likelihood function
%
...
respectively.  Then $\lambda$ would represent the effectiveness of
$p_1$'s special technique, and $\mu$ the enhancement afforded to $p_4$
by the drug; values $>1$ would correspond to a positive effect.  In
this paper I generalize the \pkg{hyper2} \proglang{R}
package~\citep{hankin2017_rnw_jss}, available on
CRAN~\citep{rcore2025_jss}, to accommodate this class of
multiplicative factors with new \proglang{S3} class \class{hyper3} and
associated methods.

The most general likelihood function for Bradley-Terry strengths
$p_i\geqslant 0, \sum p_j=1$, for a sequence of pairwise comparisons
...
\begin{equation}\label{mostgeneral}
  ...
  \left(
  \frac{
    \prod_{j\in W_i}\lambda_{ij}p_j
  }{
    \prod_{j\in S_i}\lambda_{ij}p_j
  }
  \right)^{w_i}
\end{equation}
%
where playing set $S_i$ is a subset of the competitors $\left\lbrace
1, \ldots, n\right\rbrace$ and $W_i\subseteq S_i$ is the winning set
of players on the winning side of pairwise comparison $i$.  Here the
$\lambda_{ij}\geqslant 0$ are multiplicative factors that account for
ephemeral or temporary changes in competitive ability.  The data
$w_1,\ldots w_N$ correspond to the number of ``wins'' for winning set
$W_i$ as per $w_{12},w_{34}$ in Equation~\ref{like1234}.  In practice,
...
\midrule
  ...
  \end{tabular}
\caption{The \label{oldfirm} dominance of the Old Firm: 769 matches
  between Rangers, Celtic, Falkirk and Livingston arranged by home
  wins, draws, and home losses.  Data from {\tt stats.football.co.uk}}
\end{table}
%
\begin{equation}\label{firstfewRCFL}
  ...
  \left(\frac{\lambda p_L}{\lambda p_L + p_F}\right)^{3}\cdots
  \left(\frac{p_C}{p_C + \lambda p_R}\right)^{42}\cdots
    \left(\frac{p_F}{p_F + \lambda p_L}\right)^{9}
\end{equation}
%
where the home-ground advantage is represented by~$\lambda$.  In
package idiom, a term such as $\lambda p_R + p_C$ is represented as a
named numeric vector.  For example, if the home strength were $\lambda
= 1.8$ then one bracket would be {\tt c(Rangers=1.8, Celtic=1)}
with an associated power of $-81$.  The computational methodology is a
generalization of that used in~\cite{hankin2024_hyper3_jss} in which the
multiplicative factors correspond to number of clones.  However,
in~\cite{hankin2024_hyper3_jss} the factors are known non-negative
integers, and here they are, in general, unknown non-negative real
numbers.  The \class{hyper3} class enforces \pkg{disordR}
discipline~\citep{hankin2022_disordR_jss}.

\subsection{The package in use}
Using package idiom to create a likelihood function for this dataset
is straightforward.  As an illustration, we will consider the
restriction of equation~\ref{firstfewRCFL} to just Rangers vs Celtic
matches, and ignore score draws:

\begin{equation}\label{rangers_vs_celtic}
  \mathcal{L}(p_R,p_C;\lambda)=
  \left(\frac{\lambda p_R}{\lambda p_R +         p_C}\right)^{81}\cdot
  \left(\frac{        p_R}{\lambda p_R +         p_C}\right)^{42}\cdot
  \left(\frac{\lambda p_C}{        p_R + \lambda p_C}\right)^{76}\cdot
  \left(\frac{        p_R}{        p_R + \lambda p_C}\right)^{46}
\end{equation}

Equation~\ref{rangers_vs_celtic} shows that Rangers have won 81 times
at home and 46 times away; Celtic have won 76 times at home and 42
times away.  One way of representing a likelihood function for
Rangers-Celtic matches (here, ignoring draws) would be:
%
...
assumed value $\lambda=1.88$.  This corresponds to
Equation~\ref{rangers_vs_celtic}.  We may make inferences about the
true value of $\lambda$ using profile support:
%
...
@
%
Thus \fct{f} returns the support at the evaluate, conditional on a
supplied value of $\lambda$.  We may use this to create a profile
likelihood curve (Figure~\ref{profilesupportlambda}) from which we see
strong evidence that $\lambda>1$, and find the maximum likelihood
estimate for $\lambda$:
...
\caption{A\label{profilesupportlambda} profile support function for
  ...
  $\hat{\lambda}\simeq 1.785$.  We may reject the null with about
  $\mathcal{S}\simeq 10$ units of support, exceeding the two-unit
  criterion of~\citeauthor{edwards1992}.  Alternatively, we may
  observe that $\mathcal{S}$ lies in the tail region of its asymptotic
...
@
%
These numerical results agree with the theoretical values obtained by
maximizing equation~\ref{rangers_vs_celtic}: $\widehat{p_{\mbox{\tiny
...
About 22\% of matches in Table~\ref{oldfirm} end in a draw and
\pkg{hyper2} formalism may be used to infer draw probabilities.
Consider the likelihood functions described in
Table~\ref{windrawloss}.  We see two likelihood functions: on the
left, a natural multiplicative Bradley-Terry function, and on the
right, the likelihood function of~\citet{hankin2020_jss} in which a
drawn match was considered to be won by a fictitious entity (a ``draw
monster'') who possessed his own BT strength $\mathfrak{D}$ and a
``home ground monster'' with strength $\mathfrak{H}$ representing the
home ground advantage.  However, the multiplicative likelihood
function of Table~\ref{windrawloss} has a number of desirable features
not present in the reified entity parametrization, detailed  below.

\paragraph{Identical strengths}
If we consider two players of identical strengths $p_i=p_j=p$ and
nullify the home ground effect (perhaps both teams play away), then
the reified entity probability for a draw is
$\mathfrak{D}/(2p+\mathfrak{D})$, a function of both $\mathfrak{D}$
and $p$; clearly undesirable.  Compare the multiplicative formulation,
in which the probability of a draw is $\frac{D}{1+D}$: the odds of a
draw is simply $D$, regardless of common team strength $p$.

\paragraph{Won games only}
Again with $p_i=p_j=p$, now considering only won (that is, not
score-drawn) games, the probability of a home win under the
multiplicative model is $\frac{\lambda}{1+\lambda}$: the odds of a
home win is simply $\lambda$, independently of the Bradley-Terry
strength.  This is a succinct probability model with clear
interpretation.  Compare reified Bradley-Terry, with probability
$\frac{p+\mathfrak{H}}{2p+\mathfrak{H}}$, making interpretation of
$\mathfrak{H}$ problematic.

\paragraph{One dominating team} Now consider the case where $p_i=p>0$
and $p_j=0$.  The multiplicative model gives the probability of a home
draw as $\frac{D}{\lambda + D}$ and an away draw as $\frac{D}{1+D}$;
compare the reified model's $\frac{\mathfrak{D}}{p +
  \mathfrak{D}+\mathfrak{H}}$, again not independent of the team's
strengths.

\begin{table}[t!]
\centering
\begin{tabular}{l|cc|cc}
  & \multicolumn{2}{c}{Multiplicative BT}
  & \multicolumn{2}{c}{Reified BT}
  \\&
{$i$ plays at home} &
{$i$ plays away}    &
{$i$ plays at home} &
{$i$ plays away} \\
  ...
  $\frac{p_i}{p_i + D(p_i + p_j) + \lambda p_j}$               &
  $\frac{p_i+\mathfrak{H}}{p_i+p_j+\mathfrak{D}+\mathfrak{H}}$ &
  $\frac{p_i  }{p_i+p_j+\mathfrak{D}+\mathfrak{H}}$            \\
  $\Prob{(\mbox{draw})}$                                       &
  $\frac{D(p_i + p_j)}{\lambda p_i+D(p_i+p_j) + p_j}$          &
  $\frac{D(p_i + p_j)}{p_i + D(p_i+p_j) + \lambda p_j}$        &
  $\frac{\mathfrak{D}    }{p_i+p_j+\mathfrak{D}+\mathfrak{H}}$ &
  $\frac{\mathfrak{D}    }{p_i+p_j+\mathfrak{D}+\mathfrak{H}}$ \\
  $\Prob{(\mbox{$i$ loses})}$                                  &
  $\frac{p_j}{\lambda p_i + D(p_i+p_j) + p_j}$                 &
  $\frac{\lambda p_j}{p_i + D(p_i + p_j) + \lambda p_j}$       &
  $\frac{p_j  }{p_i+p_j+\mathfrak{D}+\mathfrak{H}}$            &
  $\frac{p_j+H}{p_i+p_j+\mathfrak{D}+\mathfrak{H}}$            \\
\end{tabular}
\caption{Probabilities \label{windrawloss} of a win, draw, loss for
  two teams under the proposed multiplicative Bradley-Terry model, and
  the reified Bradley-Terry model of~\citet{hankin2020_jss}.  Observe that
  columns sum to one.  We see Bradley-Terry strengths $p_i$ and $p_j$
  for the two teams; a home ground advantage of $\lambda$
  (multiplicative) or $\mathfrak{H}$ (reified); and a propensity to
  draw $D$ (multiplicative) or $\mathfrak{D}$ (reified)}
\end{table}
%
Such arguments give direct operational significance to $\lambda$ and
$D$, when considered in isolation from BT strengths.  We are now in a
position to consider the entirety of Table~\ref{oldfirm}.  Bespoke
package function \fct{home\_draw\_away3} converts data in the form of
...
@
%
Thus \fct{f} gives us a likelihood function for
...
probability of
$\left\lbrace\mbox{win};\mbox{draw};\mbox{loss}\right\rbrace$ for
Rangers would be
$\left\lbrace\frac{p_R}{(1+D)(p_R+p_C)};\frac{D}{1+D};\frac{p_C}{(1+D)(p_R+p_C)}\right\rbrace$,
...
\caption{Contours of support\label{contourlambdaD} evaluated over
  ...
  a red dot at the evaluate.  Vertical gray line shows $H_0\colon\lambda=1$}
  \end{center}
\end{figure}


<<testlambdazero, cache = TRUE, eval = TRUE, echo = FALSE>>=
...
example, we may test the hypothesis that $\lambda=1$ [corresponding to
  the left hand side of Figure~\ref{contourlambdaD}] for which the
support is about \Sexpr{round(f(maxv)-jj$objective,2)}, far
exceeding the two units of support per degree of freedom
criterion~\citep{edwards1992}.  Alternatively, we can observe that this
lies in the tail region of its asymptotic null $\chi^2_2$ distribution
with a $p$~value of about \Sexpr{signif(pchisq(f(maxv)-jj$objective,lower.tail=FALSE,df=2),digits=2)}.

Equation~\ref{mostgeneral} is very general and may be applied to a
range of situations in which standard Bradley-Terry strengths are
modified in some way.  In the following section, I apply it to
Plackett-Luce \citep{plackett1975_jss} likelihood functions for
rank data.
...
following Plackett-Luce~\citep{plackett1975_jss} likelihood function for
this observation:
%
\begin{equation}
...
\end{equation}
%
Note the use of multiplication between the terms, indicating the
standard assumption~\citep{turner2020} of conditional independence.
Now we use the multiplicative generalization of Bradley-Terry
strengths to introduce an element of non-independence between the
terms.  There are many ways to do this, but one simple way would be to
define equivalence classes of the competitors with each equivalence
class comprising mutually supporting runners.  The metaphor would be
that a runner who has finished the race is able to support other
members of his equivalence class by cheering his still-running
teammates, boosting their performance.  The \pkg{hyper2} package
includes function \fct{cheering} which implements this functionality.
...
statistics for five competitors.  The standard Plackett-Luce
likelihoods have been modified to account for two groups of mutually
supporting runners: $\left\lbrace a,b,c\right\rbrace$ with support
term $\lambda$, and $\left\lbrace d,e\right\rbrace$ with support term
$\mu$.  From START, the first runner to cross the finishing line has
standard Plackett-Luce probability.  Taking the top path as an
example, we see that the likelihood function for $a\succ b\succ c\succ
d\succ e$ would be
...
\node at (12.3,6.8) {$d$};
\node[anchor = west] at (12.5,7) {$a\succ b\succ c\succ d\succ e$};
\node at (11.8, 7.4) {$\frac{p_d}{p_d+p_e}$};

\draw[thick] (10.6, 6) -- (11.2,6.2) -- (12.2,6.2);
\fill (12.2,6.2) circle[radius=2pt];
\node at (12.3,6.0) {$e$};
\node[anchor = west] at (12.5,6.2) {$a\succ b\succ c\succ e\succ d$};
...
\node at (12.3,5.1) {$c$};
\node[anchor = west] at (12.5,5.3) {$a\succ b\succ d\succ c\succ e$};
\node at (11.8, 5.7) {$\frac{\la{p_c}}{\la{p_c}+\ld{p_e}}$};

\draw[thick] (10.6, 5) -- (11.2,4.7) -- (12.2, 4.7);
\fill (12.2,4.7) circle[radius=2pt];
\node at (12.3,4.5) {$e$};
\node[anchor = west] at (12.5,4.7) {$a\succ b\succ d\succ e\succ c$};
...
\node at (12.3,3.7) {$c$};
\node[anchor = west] at (12.5,3.9) {$a\succ b\succ e\succ c\succ d$};
...
\node at (12.3,2.9) {$d$};
\node[anchor = west] at (12.5,3.1) {$a\succ b\succ e\succ d\succ c$};


\draw[thick] (4.8, 0) -- (5.8,1) -- (8.2,1);
\fill (12.2,1) circle[radius=2pt];
\node at (12.3,0.8) {$e$};
\node[anchor = west] at (12.5,1.0) {$d\succ a\succ c\succ e\succ b$};
\node at (7, 1.4)  {$\frac{p_a}{p_a+p_b+p_c+\ld{p_e}}$};

\draw[thick] (4.8, 0) -- (5.8,0) -- (8.2,0);
\fill (12.2,0) circle[radius=2pt];
\node at (12.3, -0.2) {$b$};
\node[anchor = west] at (12.5, 0) {$d\succ e\succ a\succ b\succ c$};
...
\fill (12.2,-1) circle[radius=2pt];
\node at (12.3,-1.2) {$c$};
\node[anchor = west] at (12.5,-1) {$d\succ e\succ a\succ c\succ b$};
...
\node at (12.3, -2.0) {$a$};
\node[anchor = west] at (12.5, -1.8) {$d\succ e\succ b\succ a\succ c$};
\node at (11.8, -1.4) {$\frac{\la{p_a}}{\la{p_a}+\la{p_c}}$};

\draw[thick] (10.6, -2) -- (11.2,-2.6) -- (12.2,-2.6);
\fill (12.2,-2.6) circle[radius=2pt];
\node at (12.3, -2.8) {$c$};
\node[anchor = west] at (12.5,-2.6) {$d\succ e\succ b\succ c\succ a$};
...
\node at (12.3, -3.6) {$b$};
\node[anchor = west] at (12.5,-3.4) {$d\succ e\succ c\succ b\succ a$};
...
\node at (12.3, 1.8) {$b$};
\node[anchor = west] at (12.5, 2) {$d\succ a\succ c\succ b\succ e$};
\draw[thick] (10.6, 1) -- (11.2,1) -- (12.2,1);
...
\frac{\lambda p_c}{\lambda p_b + \lambda p_c + \mu p_e}\cdot
...
And of course one might conceptualise still-running competitors
supporting one another.  There is no reason that $\lambda$ could not
be less than one, which would indicate that some competitors inhibit,
rather than help, others.
...
of different types of sushi, and one in the field of esports.  First,
however, we discuss a synthetic example showing how generalizing
Plackett-Luce to admit non-independence of successive arrivals can be
applied to an apparently different situation.

\subsection{Red bus, blue bus}
...
for the same phenomenon by \cite{tverskey1972_jss}.  The canonical
problem is as follows.  A commuter has a choice of going to work by
different transport methods.  Here, we can assume that he has possibly
strong---but unknown---views on different forms of transport which
might incorporate a random component; we represent the probabilities
using BT strengths $p_C, p_{RB}, p_{BB}$ (for car and red/blue bus
respectively).  Our respondent states that the colour of the bus is of
no importance to him.

If our respondent is considering whether to use a car or a blue bus
[we say that the {\em choice set} is $\left\lbrace p_C,
  p_{BB}\right\rbrace$], and our respondent chooses car with
probability one-half, $\Prob(C | \left\lbrace
C,BB\right\rbrace)=\frac{1}{2}$, we may infer that $p_C=p_{BB}$.  We
now present a red bus with strength $p_{RB}$; his stated indifference
to bus color gives us $\Prob(RB|\left\lbrace
RB,BB\right\rbrace)=\frac{1}{2}$, so we infer that $p_{RB}=p_{BB}$,
and thus $p_{C}=p_{BB}=p_{RB}$.  Any realistic choice model will have
$\Prob(C | \left\lbrace C,RB,BB\right\rbrace)=\frac{1}{2}$ but
Bradley-Terry gives us $\frac{1}{3}$.

Various solutions to this problem have been proposed.
\cite{tverskey1972_jss}, for example, presents an approach involving
overlapping ``aspects'' of the choice set; \cite{mcfadden1980}
suggests using a generalized extreme-value model.  Here I suggest an
alternative in the form of a one-parameter generalization of
...
above.  As a synthetic example, we specify options RB and BB for red
bus and blue bus together with options C, T and W for car, train, and
walking.

Specifically, it seems reasonable to assume that if we ask our
respondent to rank his options, it is highly probable that he will put
RB and BB consecutively (because they are essentially
indistinguishable).  Can we quantify the strength of this effect?  To
do this, we define a bespoke function \fct{RB\_BB\_LF} which
returns a \code{hyper3} log-likelihood function corresponding to
...
ranks RB and BB consecutively.  Function \fct{RB\_BB\_LF} takes
argument \code{lambda} that quantifies the perceived similarity
between RB and BB in the same way as function \fct{oldfirm} above.
%
<<definefunctiono>>=
o <- function(lambda){maxp(RB_BB_LF(lambda), give = 1)$`Log-likelihood`} 
@ 
%
Above, we define function \fct{o} which returns the support for a
given value of $\lambda$ at the evaluate [over permissible Bradley
  Terry strengths for the options].  We can treat \fct{o} as a
...
with a $p$~value of \Sexpr{signif(pchisq(suppdiff*2,df=1,lower.tail=FALSE),3)}.
%
...
would be modelled by having $\lambda<1$.  Implementation in
\code{hyper3} formalism is straightforward.  The dataset used is
\code{sushi\_table}, part of the \pkg{hyper2} package:
%
<<showsushitable>>=
<<echo = FALSE>>=
rownames(sushi_table) <- rep("", 50)
<<echo = TRUE>>=
...
@ 
%
% chunk 'findstuffsushi' takes about 10-20 minutes to run
%
<<findstuffsushi, echo = FALSE, cache = TRUE>>=
osushi <- optimize(f, c(1, 4), maximum = TRUE)
@ 
%
% chunk tinysushi' takes about 5 minutes to run
%
<<tinysushi, echo = FALSE, cache = TRUE>>=
sushimaxlambda <- osushi$maximum
sushisupport <- osushi$objective - f(1)
@
%
% chunk 'findotherstuffsushi' takes about 10 minutes to run
%
...
@ 
%
(above, we note that the return value {\tt H} is the sum of successive
log-likelihood functions of each row of the table, assumed to be
independent).  Then, as before, we may define a profile likelihood
function of $\lambda$, Figure~\ref{profilesupportsushi}.  We
reject~$H_0\colon\lambda=1$ in favour of
$H_A\colon\lambda=\Sexpr{round(sushimaxlambda,2)}$ with about
$\Sexpr{round(sushisupport,2)}$ units of support.  Alternatively, we
may observe that this lies in the tail region of its asymptotic
$\chi^2_1$ distribution with a $p$~value of about
$\Sexpr{round(pchisq(2*sushisupport,df=1,lower.tail=FALSE),4)}$.  We can
therefore be reasonably sure that tuna sushi are somehow analogous to
red and blue buses, in the sense that all three tuna items tend to
occupy closely separated places in judges' preference ranks.
...
\caption{A\label{profilesupportsushi} profile support function for the
  ...
  $\lambda\simeq\Sexpr{round(sushimaxlambda,2)}$}
\end{center}
\end{figure}
...
Here I analyse a dataset taken from recent esports tournament {\em
...
%
<<display_boston2018_dataset, echo = FALSE>>=
a <- read.table("cloud9_boston_2018.txt", header = TRUE)[,-1]
rownames(a) <- NULL
<<echo = TRUE, eval = FALSE>>=
head(cloud9_boston_2018)
...
situations might result in a systematic preference for targeting
...
strength.  The \pkg{hyper2} \proglang{R} package implements the
resulting likelihood functions using \proglang{S3} class {\tt hyper3},
and furnishes a wide range of functionality geared towards assessing
meaningful nulls using likelihood methods.
...
non-independence schemes.

Further work might include an individualised multiplicative draw
factor, giving rise to likelihood functions such as $\frac{\lambda_i
  p_i;D_i p_i + D_j p_j; \lambda_j p_j}{\lambda_i p_i + D_i p_i + D_j
  p_j + \lambda_j p_j}$.  One might also consider multiplicative
factors for the strengths of {\em reified} entities.  These
modifications would be directly representable with existing {\tt
  hyper3} idiom.
