...
Implementation in \code{hyper3} formalism is straightforward.  Object
\code{sushi\_eq\_class} specifies the two equivalence classes
...
We can now use this to define a bespoke likelihood function, using
...
Then, as before, we may define a profile likelihood function of $\lambda$:
