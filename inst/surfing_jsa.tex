...
\usepackage[pdftex]{hyperref}
  ...
  \author{Driver, Timothy${}^{1,2}$ \and
    Hankin, Robin K. S.${}^{2,3}$ \\
    \texttt{hankin.robin@gmail.com}\\(corresponding author)}


  \date{${}^1$ {\bf Tim what do you want here you can put anything}\\
...
The Bradley-Terry model \citep{bradley1954} assigns non-negative strengths
...
p_i=1$.  Further, we use a generalization due to \citep{luce1959}, in which
...
just the first across the line, we can follow \citet{plackett1975} and
...
{\tt wct01}, Colapinto came first, Bailey second, and Wright came
third.  The last line shows that, at {\tt wct11}, Ferreira came first
and Medina second.  We can convert this dataset into a Plackett-Luce
likelihood function but first have to remove the wildcards and also
player Vries whose maximum likelihood strength is zero.  For the first
line we would have Plackett-Luce likelihood function as
...
to describe the fact that six out of eight querterfinalists at the 2011 Nike
...
\subsection{Brazilian wave type prefereneces}
...
that Brazilians' performce is better on their preferred wave types
...
man out.  Such hypotheses are difficult to test with conventional
techniques as competitors's strengths themselves are nuisance
parameters~\citep{basu1977}.
...
bolster intranational ranking at the expense WCT performance.

Further work might include statistical investigation of noncompetitive
NASH STUFF compatriot priority infringements between compatriots.
