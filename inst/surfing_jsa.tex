...
\begin{equation}
  ...
  \begin{cases}
    -n/\left(p_i+p_j+\sum p_k\right)^2\qquad\mbox{if $i,j\in K$}\\
...
Surf Ranch.  To do this, we create an artificial surfer, {\tt
  ...
  Medina} has the same Plackett-Luce strength as {\tt Medina\_wct08}.
...
\begin{table}[h]
\begin{tabular}{l|l|l}
wave type  & $\operatorname{Prob}(p_1\succ p_2)$ & $\operatorname{Prob}(p_2\succ p_1)$\\ \hline
beach & $\frac{p_1+B}{p_1+p_2+B}$ & $\frac{p_2}{p_1+p_2+B}$\\
point & $\frac{p_1+B}{p_1+p_2+B}$ & $\frac{p_2}{p_1+p_2+B}$\\
reef  & $\frac{p_1\vphantom{B}}{p_1+p_2}$ & $\frac{p_2}{p_1+p_2}$\\
\end{tabular}
