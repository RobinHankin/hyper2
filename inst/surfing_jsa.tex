...
(this document is based on \citep{driver2023}).
...
amenable to analysis via general linear models~\citep{turner2012_nomarkup}.
...
Fig.~\ref{dotchartmax} shows the maximum likelihood estimate for the
...
likelihoods.  Fig.~\ref{compare_likelihood_points} shows a loose
correlation ($R^2=0.71$; $p=4\times 10^{-7}$) and
Fig.~\ref{compare_likelihood_points_rankings} shows how the rankings
...
0.015 (Fig.~\ref{brazilian_wavetype_support}): at this value of the
...
% use   \begin{tabular}{@{}c@{}}\small{WCT} \\ 01\end{tabular} for WCT\\01
\begin{table}[h]
...
$p_1\succ p_2\succ q $ \hphantom{www}
                       & $ \frac{p_1}{p_1+p_2+q+p_C}\cdot\frac{p_2^{\vphantom{h}}}{p_2+q+p_C}$\\
...
\begin{figure}[h]
  \includegraphics[width=4in]{directplotofstrengths_postinkscape.pdf}  % to create this file, open directplotofstrengths_postinkscape.svg in inkscape and export to PDF.  File directplotofstrengths_postinkscape.svg is under version control in the hyper2 github repo.  It is a slight modification of directplotofstrengths.pdf, with slight optical glitches removed.  File directplotofstrengths.pdf is produced by surfing.Rmd.
