...
played up to and including that round for each of the 8 players.  Each
(named) column corresponds to a player.  Looking at the last row of
`cumulative_noncollusive_matches` we see that in round 28, players
Petrosian, Keres, and Geller had played a total of 18 noncollusive
matches while the other players had played considerably more and would
...
```{r}
aa <- subset(a,a[,5] != a[,6])
aa <- subset(aa,aa[,3] != "1/2-1/2")

M <- table(aa[,3],white_rested=aa[,5]<aa[,6])
M
