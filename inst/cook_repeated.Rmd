...
P <- hyper2(pnames=c(cooks,"boy","girl"))
P0 <- hyper2(pnames=cooks)
for(i in seq_len(nrow(cooking_table))){  # iterate over rows
    x <- cooking_table[i,] 
	print(x)
	if(sum(x==0)>1){break}
	...
	if(names(x[x==0]) %in% boys){  # if the voter is a boy
		...
		Pjj <- order_obs(P0,x) # order statistic
		Pjj %<>% pwa(names(x)[names(x) %in% boys], chameleon="boy")
		pnames(Pjj) <- c(cooks,"boy","girl")
	} else if (names(x[x==0]) %in% girls){ # voter a girl
		x <- x[x>0]
		Pjj <- order_obs(P0,x)
		Pjj %<>% pwa(names(x)[names(x) %in% girls], chameleon="girl")
		pnames(Pjj) <- c(cooks,"boy","girl")
	} else {
		stop()
	}
