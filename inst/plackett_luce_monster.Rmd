...
`RB_BB_LF` <- function(s){
    ...
    (
        cheering3(v=c("RB","BB","C" ,"T","W"),e=ec,h=h)*7 + 
        cheering3(v=c("BB","RB","T" ,"C","W"),e=ec,h=h)*4 + 
        cheering3(v=c("T" ,"BB","RB","C","W"),e=ec,h=h)*5 + 
        cheering3(v=c("W" ,"BB","RB","T","C"),e=ec,h=h)*8 + 
        cheering3(v=c("C" ,"RB","BB","W","T"),e=ec,h=h)*9 + 
        cheering3(v=c("BB","C" ,"RB","T","W"),e=ec,h=h)*5
    )
}
```

Above, we see from the function body that he reported `RB,BB,C,T,W`
seven times, `BB,RB,T,C,W` four times, and so on; perhaps his ranking
depends on the weather or how tired he is on any given day.  Observe
that in almost every case he ranks `RB` and `BB` consecutively.
Function `RB_BB_LF()` takes argument `s` that quantifies the perceived
similarity between `RB` and `BB`.  For example:
...
s <- exp(seq(from=log(1.3),to=log(37),len=17)) # putative similarity measures
L <- sapply(s,o)
L <- L-max(L)
```
...
(suppdif <- o(osup$maximum) - o(1))
```


Falling just shy of Edwards's two-units-of-support criterion.
