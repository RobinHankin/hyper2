...
the _loser_ (which is here taken to be player `a`).  We have six
competitors `a,b,c,d,e,f` with nonnegative strengths
$p_1,p_2,p_3,p_4,p_5,p_6, \sum p_i=1$.  Competitor `a` has strength
$p_1$, and loses.  We are interested in how informative this
observation is with regard to the hypothesis that `a` does in fact
have zero strength.  We have three hypotheses:
...
In the above, `L` is a likelihood function for the observation that
competitor `a` came last.  In R idiom, it is a list of hyper2 objects
corresponding to all $5!=120$ possible observed order statistics (only
the first two, `L[[1]]` and `L[[2]]`, are printed, which correspond to
orders `bcdefa` and `bcdfea` respectively).  Any order is permissible,
so long as player `a` is bottom.
...
intuitive grounds: $H_3$ says that all players are equivalent and if
this is correct, then one would identify players `a` as the worst with
a probability of $\frac{1}{n}$).  Note again that this is loose
language as likelihood is only defined up to a multiplicative
constant.

# Discussion

We are now in a position to define a likelihood function for
$H-1,H_2,H_3$:
...
observation that I come last.  This is exactly the same as the
likelihood ratio for the following situation: we toss a coin which is
either fair ($H_\mbox{fair}$) or two-headed ($H_\mbox{2heads}$).  We
observe a head.  Then ${\mathcal L}(H_\mbox{2heads})/{\mathcal
L}(H_\mbox{fair}) = 0.5$ as well (this is Edwards's valet and black
and white ball thought experiment).
